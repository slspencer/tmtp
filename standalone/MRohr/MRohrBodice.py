...
class PatternDesign():
    ...
    def pattern(self):
        ...
        # Front Bodice
        bf.add(Point('reference', 'A', ( bf.start.x + cd.bust_circumference*(0.5) ) + (4*in_to_px), bf.start.y, 'point_style'))  # 4 in is to give room for seam allowances between patterns
        bf.add(Point('reference', 'B', bf.A.x, bf.A.y + cd.back_shoulder_height, 'point_style'))
        bf.add(Point('reference', 'C', bf.A.x - (cd.across_back*(0.5) + (in_to_px*3/8) ), bf.A.y, 'point_style'))
        bf.add(Point('reference', 'D', bf.C.x, bf.C.y + cd.back_shoulder_slope_depth, 'point_style'))

        a = cd.back_shoulder_slope_depth
        debug(a)
        debug(cd.shoulder_seam)
        c = cd.shoulder_seam + (in_to_px*1/4)  # 1/4" ease
        debug(c)
        b = math.sqrt(abs(cd.shoulder_seam**2) - (a**2))
        debug(b)
        x,  y = bf.D.x + b,  bf.A.y
