#!/usr/bin/python
#
# Steampunk Pattern Inkscape extension
# steampunk_jacket.py
# Copyright:(C) Susan Spencer 2010
#
# This program is free software; you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation; either version 2 of the License, or
# (at your option) any later version.

import sys, copy
import inkex
import re
import simplestyle
import simplepath
import simpletransform
import math
import lxml
import xml
import py2geom
from lxml import objectify
from scour import removeNamespacedAttributes as removeNSAttrib
from scour import removeNamespacedElements as removeNSElem

# define directory where this script and steampunk_jacket.inx are located
sys.path.append('/usr/share/inkscape/extensions')

###############################
######## Define globals #######
###############################
# Namespaces dictionary
NSS = {
       u'cc'       :u'http://creativecommons.org/ns#',
       u'rdf'      :u'http://www.w3.org/1999/02/22-rdf-syntax-ns#',
       u'svg'      :u'http://www.w3.org/2000/svg',
       u'sodipodi' :u'http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd',
       u'inkscape' :u'http://www.inkscape.org/namespaces/inkscape',
       u'xml'      :u'http://www.w3.org/XML/1998/namespace',
       u'xmlns'    :u'http://www.w3.org/2000/xmlns/',
       u'xlink'    :u'http://www.w3.org/1999/xlink',
       u'xpath'    :u'http://www.w3.org/TR/xpath',
       u'xsl'      :u'http://www.w3.org/1999/XSL/Transform'
        }

# pattern name
pattern_name = 'Steampunk Jacket'
pattern_number = 'MJ1870_1'

# measurement constants
in_to_px = ( 90 / 1 )                    #convert inches to pixels - 90px/in
cm_to_in = ( 1 / 2.5 )               #convert centimeters to inches - 1in/2.5cm
cm_to_px = ( 90 / 2.5 )              #convert centimeters to pixels

in_to_pt = ( 72.72 / 1 )                 #convert inches to printer's points - 72.72pt = 1in
cm_to_pt = ( 72.72 / 2.5 )              #convert centimeters to pixels
border   = ( 3 * in_to_pt)           # 3" document borders


# sewing constants
quarter_seam_allowance = in_to_pt * 1 / 4    # 1/4" seam allowance
seam_allowance         = in_to_pt * 5 / 8    # 5/8" seam allowance  
hem_allowance          = in_to_pt * 2        # 2" seam allowance
pattern_offset         = in_to_pt * 4        # 4" between patterns   

SVG_OPTIONS = {  'width' : "auto",
                'height' : "auto",
          'currentScale' : "0.05 : 1",
      'fitBoxtoViewport' : "True",
   'preserveAspectRatio' : "xMidYMid meet",
         'margin-bottom' : str(3*cm_to_pt),
           'margin-left' : str(3*cm_to_pt),
          'margin-right' : str(3*cm_to_pt),
            'margin-top' : str(3*cm_to_pt),   
          'pattern-name' : "Steampunk Jacket"
                }

class DrawJacket( inkex.Effect ):

    def __init__(self):

          inkex.Effect.__init__( self, **SVG_OPTIONS ) 
   
          # Store user measurements from steampunk_jacket.inx into object 'self'  
          OP = self.OptionParser              # use 'OP' make code easier to read
          OP.add_option('--measureunit', 
                        action='store', 
                        type='str', 
                        dest='measureunit', 
                        default='cm', 
                        help='Select measurement unit:')
          OP.add_option('--height', 
                         action='store', 
                         type='float', 
                         dest='height', 
                         default=1.0, 
                         help='Height in inches') 
          OP.add_option('--chest', 
                         action='store', 
                         type='float', 
                         dest='chest', 
                         default=1.0, 
                         help='chest')
          OP.add_option('--chest_length', 
                         action='store', 
                         type='float', 
                         dest='chest_length', 
                         default=1.0, help='chest_length')
          OP.add_option('--waist', 
                         action='store', 
                         type='float', 
                         dest='waist', 
                         default=1.0, 
                         help='waist')
          OP.add_option('--back_waist_length', 
                         action='store', 
                         type='float', 
                         dest='back_waist_length', 
                         default=1.0, 
                         help='back_waist_length') 
          OP.add_option('--back_jacket_length', 
                         action='store', 
                         type='float', 
                         dest='back_jacket_length', 
                         default=1.0, help='back_jacket_length')
          OP.add_option('--back_shoulder_width', 
                         action='store', 
                         type='float', 
                         dest='back_shoulder_width', 
                         default=1.0, 
                         help='back_shoulder_width')
          OP.add_option('--back_shoulder_length', 
                         action='store', 
                         type='float', 
                         dest='back_shoulder_length', 
                         default=1.0, 
                         help='back_shoulder_length')
          OP.add_option('--back_underarm_width', 
                         action='store', 
                         type='float', 
                         dest='back_underarm_width', 
                         default=1.0, 
                         help='back_underarm_width')
          OP.add_option('--back_underarm_length', 
                         action='store', 
                         type='float', 
                         dest='back_underarm_length', 
                         default=1.0, 
                         help='back_underarm_length')
          OP.add_option('--seat', 
                         action='store', 
                         type='float', 
                         dest='seat', 
                         default=1.0, 
                         help='seat') 
          OP.add_option('--back_waist_to_hip_length', 
                         action='store', 
                         type='float', 
                         dest='back_waist_to_hip_length', 
                         default=1.0, help = 'back_waist_to_hip_length')
          OP.add_option('--nape_to_vneck', 
                         action='store', 
                         type='float',                          
                         dest='nape_to_vneck',
                         default=1.0,
                         help='Nape around to about 11.5cm (4.5in) below front neck')
          OP.add_option('--sleeve_length', 
                         action='store', 
                         type='float', 
                         dest='sleeve_length', 
                         default=1.0, help='sleeve_length') 

    def Debug( self, msg ):
           sys.stderr.write( str( msg ) + '\n' )
           return msg

    def GetDot(self, layer, x, y, name):
           style = {   'stroke'       : 'red',  
                       'fill'         : 'red',
                       'stroke-width' : '8' }
           attribs = { 'style'        : simplestyle.formatStyle( style ),
                        inkex.addNS( 'label', 'inkscape' ) : name,
                        'cx'           : str(x),
                        'cy'           : str(y),
                        'r'           : str( (.05) * in_to_pt ) }
           inkex.etree.SubElement( layer, inkex.addNS( 'circle', 'svg' ),  attribs )
           return x, y, str(x) + ',' + str(y)

    def GetCircle(self, layer, x, y, radius, color, name, ):
           style = {   'stroke'       : color,  
                       'fill'         :'none',
                       'stroke-width' :'6' }
           attribs = { 'style'        : simplestyle.formatStyle( style ),
                        inkex.addNS( 'label', 'inkscape' ) : name,
                        'cx'          : str(x),
                        'cy'          : str(y),
                        'r'           : str(radius) }
           inkex.etree.SubElement( layer, inkex.addNS( 'circle', 'svg' ), attribs )
   
    def sodipodi_namedview(self):
           svg_root   = self.document.xpath('//svg:svg', namespaces = inkex.NSS)[0]
           #sodi_root = self.document.xpath('//sodipodi:namedview', namespaces = inkex.NSS)[0]
           #ink_root  = self.document.xpath('//sodipodi:namedview/inkscape',  namespaces = inkex.NSS)
 
           #root_obj = lxml.objectify.Element('root')
           #sodi_obj = lxml.objectify.Element('namedview')
           #attrList = namedview.Attributes()
	   #for i in range(attrList.length):
	   #   attr = attrList.item(i)
	   #   setAttributeNS( attr.namespaceURI, attr.localName, attr.nodeValue)
	   # sodi_root.removeElement('inkscape:window-y')
           # sodi_root.setElement('window-y','')
           # del sodi_root.inkscape:window-y
           # del ink_root.window-y.attribs

           attribs = {  inkex.addNS('sodipodi-insensitive')        : 'false', 
                        'bordercolor'                              : "#666666",
                        'borderopacity'                            : "0.5",
                        'id'                                       : 'sodipodi_namedview',
                        inkex.addNS('current-layer','inkscape')    : 'layer1',
                        inkex.addNS('cy','inkscape')               : "9.3025513",
                        inkex.addNS('cx','inkscape')               : "9",
                        inkex.addNS('document-units','inkscape')   : "cm",
                        inkex.addNS('pageopacity','inkscape')      : "0.0",
                        inkex.addNS('pageshadow','inkscape')       : "1",
                        inkex.addNS('window-height','inkscape')    : "800",
                        inkex.addNS('window-maximized','inkscape') : "1", 
                        inkex.addNS('window-width','inkscape')     : "1226",
                        inkex.addNS('window-y','inkscape')         : "26",
                        inkex.addNS('window-x','inkscape')         : "42",
                        inkex.addNS('zoom','inkscape')             : "16",
                        'pagecolor'                                : "#fffaaa",
                        'showgrid'                                 : 'false'
                     }
           inkex.etree.SubElement( svg_root, inkex.addNS( 'namedview', 'sodipodi'), attribs)

           #svg = doc.getroot()
           #group = inkex.etree.Element(inkex.addNS('g','svg'))
           #in-width = inkex.unittouu(svg.get('width'))
           #in-height = inkex.unittouu(svg.get('height'))
           #in-view = map(inkex.unittouu, svg.get('viewBox').split())
           #group.set('transform',"translate(%f,%f) scale(%f,%f)" % (-in-view[0], -in-view[1], scale*in-width/(in-view[2]-in-view[0]), scale*in-height/(in-view[3]-in-view[1])))

    def svg_svg(self, width, height, border ):

           svg_root  = self.document.xpath('//svg:svg', namespaces = inkex.NSS)[0]   
           svg_root.set( "width",  '100%' )
           svg_root.set( "height", '100%' )
           svg_root.set( "x", '0%' )
           svg_root.set( "y", '0%' )
           svg_root.set( "currentScale", "0.05 : 1") 
           svg_root.set( "fitBoxtoViewport", "True") 
           svg_root.set( "preserveAspectRatio", "xMidYMid meet")
           svg_root.set( "zoomAndPan", "magnify" )
           svg_root.set( "viewBox", '0 0 '+str(width) +' '+ str(height) )

           # define 3-inch document borders   --> works 
           border = str(  3 * in_to_pt )
           svg_root.set( "margin-bottom", border ) 
           svg_root.set( "margin-left",   border ) 
           svg_root.set( "margin-right",  border ) 
           svg_root.set( "margin-top",    border ) 

           # add pattern name & number                --> works     
           svg_root.set( "pattern-name", pattern_name )
           svg_root.set( "pattern-number", pattern_number ) 

           # set document center --> self.view_center
           xattr = self.document.xpath('//@inkscape:cx', namespaces=inkex.NSS )  
           yattr = self.document.xpath('//@inkscape:cy', namespaces=inkex.NSS )
           if xattr[0] and yattr[0]:
               #self.view_center = (float(xattr[0]),float(yattr[0]))    # set document center
               self.view_center = ( ( float(width) / 2 ),( float(height) / 2 ) )    # set document center
           self.Debug(self.view_center)
           self.Debug(xattr)
           self.Debug(yattr)
  
           #x = self.document.xpath('//@viewPort', namespaces=inkex.NSS)
           #viewbox='0 0 '+widthstr+' '+heightstr
           #root.set("viewBox", viewbox)      # 5 sets view/zoom to page width
           #root.set("width","auto")  #doesn't work
           #x = self.document.location.reload()
           #root.set("width", "90in" % document_width)
           #root.set("height", "%sin" % document_height)
           #x.set("width",widthstr(border*2 + self.options.back_shoulder_width   

    def GetNewLayer(self,mylayer,name):
           self.layer = inkex.etree.SubElement( mylayer, 'g' )
           self.layer.set( inkex.addNS( 'label', 'inkscape'), name )
           self.layer.set( inkex.addNS( 'layer', 'inkscape'), name + ' Layer' )
           self.layer.set( inkex.addNS( 'groupmode', 'inkscape'), 'group' )
           return self.layer

    def Path(self,layer,pathdefinition,pathtype,name,trans):
           if (pathtype=='reference'):
               style = { 'fill':'none',
                         'stroke':'gray',
                         'stroke-width':'6',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4',
                         'stroke-dasharray':'6,18',
                         'stroke-dashoffset':'0'}
           elif (pathtype=='line'):
               style = { 'fill':'none',
                         'stroke':'pink',
                         'stroke-width':'7',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4'}
           elif (pathtype=='dart'):
               style = { 'fill':'none',
                         'stroke':'gray',
                         'stroke-width':'7',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4'}
           elif (pathtype=='fold'):
               style = { 'fill':'none',
                         'stroke':'gray',
                         'stroke-width':'4',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4'}
           elif (pathtype=='grainline'):
               style = { 'fill':'none',
                         'stroke':'green',
                         'stroke-width':'8',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4',
                         inkex.addNS('marker-start','svg'):'url(#Arrow2Lstart)',
                         'marker-end':'url(#Arrow2Lend)'}
           elif (pathtype=='seam'):
               style = { 'fill':'none',
                         'stroke':'green',
                         'stroke-width':'6',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4',
                         'stroke-dasharray':'24,6',
                         'stroke-dashoffset':'0'}
           else:
               style = { 'fill':'none',
                         'stroke':'green',
                         'stroke-width':'8',
                         'stroke-linejoin':'miter',
                         'stroke-miterlimit':'4'}                              
           pathattribs = { inkex.addNS('label','inkscape') : name,
                          'transform' : trans,
                          'd': pathdefinition, 
                          'style': simplestyle.formatStyle(style)}
           inkex.etree.SubElement(layer, inkex.addNS('path','svg'), pathattribs)
           
    def XY(self,x,y,px,py,length):
           # x,y is point to measure from to find XY
           # if mylength>0, XY will be extended from xy away from pxpy
           # if mylength<0, XY will be between xy and pxpy
           # xy and pxpy cannot be the same point
           # otherwise, .   px,py are points on existing line with x,y
           # line slope formula:     m = (y-y1)/(x-x1)
           #                        (y-y1) = m(x-x1)                         /* we'll use this in circle formula
           #                         y1 = y-m(x-x1)                          /* we'll use this after we solve circle formula
           # circle radius formula: (x-x1)^2 + (y-y1)^2 = r^2                /* see (y-y1) ? 
           #                        (x-x1)^2 + (m(x-x1))^2 = r^2             /* substitute m(x-x1) from line slope formula for (y-y1) 
           #                        (x-x1)^2 + (m^2)(x-x1)^2 = r^2           /* distribute exponent in (m(x-x1))^2
           #                        (1 + m^2)(x-x1)^2 = r^2                  /* pull out common term (x-x1)^2 - advanced algebra - ding!        
           #                        (x-x1)^2 = (r^2)/(1+m^2)
           #                        (x-x1) = r/sqrt(1+(m^2))
           #                         x1 = x-r/sqrt(1+(m^2))
           #                      OR x1 = x+r/sqrt(1+(m^2))
           # solve for (x1,y1)
           m=self.Slope(x,y,px,py,'normal')
           r=length
           #solve for x1 with circle formula, or right triangle formula
           if (m=='undefined'):
               x1=x
               if (py <= y):
                  y1=y+r
               else:
                  y1=y-r
           else:
               if (m==0):
                   y1=y
                   if (px <= x):
                       x1=x+r
                   else:
                       x1=x-r
               else:
                   if (px <= x):
      	               x1=(x+(r/(self.Sqrt(1+(m**2)))))
                       y1=y-m*(x-x1)                        #solve for y1 by plugging x1 into point-slope formula             
                   else:
      	               x1=(x-(r/(self.Sqrt(1+(m**2)))))
                       y1=y-m*(x-x1)                        #solve for y1 by plugging x1 into point-slope formula             
           return x1,y1

    def LineLength(self,ax,ay,bx,by):
           #a^2 + b^2 = c^2
           c_sq= ((ax-bx)**2) + ((ay-by)**2)
           c=self.Sqrt(c_sq)
           return c

    def XYwithSlope(self,x,y,px,py,length,slopetype):
           # x,y the point to measure from, px&py are points on the line, mylength will be appended to x,y at slope of (x,y)(px,py)
           # mylength should be positive to measure away from px,py, or negative to move towards px,py
           # this function returns x1,y1 from the formulas below
           # --->to find coordinates 45degrees from a single point, add or subtract N from both x & y. I usually use 100, just over an inch.
           #     for finding point 2cm 45degrees from x,y, px=x+100, py=y-100 (Inkscape's pixel canvas's y decreases as you go up, increases down. 
           #     0,0 is upper top left corner.  Useful for finding curves in armholes, necklines, etc.
           #     check whether to add or subtract from x and y, else x1,y1 might be in opposite direction of what you want !!! 
           # 
           # line slope formula:     m = (y-y1)/(x-x1)
           #                        (y-y1) = m(x-x1)                         /* we'll use this in circle formula
           #                         y1 = y-m(x-x1)                          /* we'll use this after we solve circle formula
           #
           # circle radius formula: (x-x1)^2 + (y-y1)^2 = r^2                /* see (y-y1) ? 
           #                        (x-x1)^2 + (m(x-x1))^2 = r^2             /* substitute m(x-x1) from line slope formula for (y-y1) 
           #                        (x-x1)^2 + (m^2)(x-x1)^2 = r^2           /* distribute exponent in (m(x-x1))^2
           #                        (1 + m^2)(x-x1)^2 = r^2                  /* pull out common term (x-x1)^2 -     
           #                        (x-x1)^2 = (r^2)/(1+m^2)
           #                        (x-x1) = r/sqrt(1+(m^2))
           #                         x1 = x-(r/sqrt(1+(m^2)))                /* if adding to left end of line, subtract from x
           #                      OR x1 = x+(r/sqrt(1+(m^2)))                /* if adding to right end of line, add to x
           # solve for (x1,y1)
           r=length
           if (x!=px):
               m=self.Slope(x,y,px,py,slopetype)
               if (m==0):
                   y1=y
                   if (px <= x):
                       x1=x+r
                   else:
                       x1=x-r
               else:
                   m_sq=(m**2)
                   sqrt_1plusm_sq=self.Sqrt(1+(m_sq))
                   if (px <= x):
      	               x1=(x+(r/sqrt_1plusm_sq))        #solve for x1 with circle formula, or right triangle formula  
                   else:
      	               x1=(x-(r/sqrt_1plusm_sq))
                   y1=y-m*(x-x1)                        #solve for y1 by plugging x1 into point-slope formula         
           elif  (slopetype=='normal') or (slopetype=='inverse'):
               if (slopetype=='inverse'):
                   x1=-x
               else:
                   x1=x
               if (py <= y):
                  y1=y+r
               else:
                  y1=y-r
           else:    #perpendicular to undefined slope where x==px, so return points on horizontal slope=0 y
               y1=y
               if (px<=x):
                  x1=x+r
               else:
                  x1=x-r   
           return x1,y1

    def Slope(self,x1,y1,x2,y2,slopetype):
           # slopetype can only be {'normal','inverse','perpendicular'}
           if ((slopetype=='normal') or (slopetype=='inverse')):
               if (x1==x2):
                   slope='undefined'
               elif (y2==y1):
                   slope=0    #force this to 0, Python might retain as a very small number
               if (slopetype=='inverse'):
                   slope=-((y2-y1)/(x2-x1))
               else:
                   slope=((y2-y1)/(x2-x1))
           else:    #perpendicular slope -(x2-x1)/(y2-y1)
               if (x1==x2):
                   slope='0'
               elif (y2==y1):
                   slope='undefined'
               else:
                   slope=-((x2-x1)/(y2-y1))      
           return slope

    def AngleFromSlope(self, rise, run):
        # works with both positive and negative values of rise and run
        # returns angle in radians
        return math.atan2(rise, run)

    def NewPointFromDistanceAndAngle(self, x1, y1, distance, angle):
        # http://www.teacherschoice.com.au/maths_library/coordinates/polar_-_rectangular_conversion.htm
        x2 = x1 + (distance * math.cos(angle))
        y2 = y1 - (distance * math.sin(angle))
        return (x2, y2)

    def Intersect(self,x11,y11,x12,y12,x21,y21,x22,y22):
           # y=mx+b  --> looking for point x,y where lp_midpoint*x+b1=lp_top_left*x+b2
           # b1=y1-lp_midpoint*x1
           # !!!!!!!!!!!!Test later for parallel lines  and vertical lines
           lp_midpoint=self.Slope(x11,y11,x12,y12,'normal')
           if (lp_midpoint=='undefined'):
               x=x11
           #else:
           b1=(y11-(lp_midpoint*x11))
           # b2=y2-lp_top_left*x2
           lp_top_left=self.Slope(x21,y21,x22,y22,'normal')
           #if (lp_top_left=='undefined'):
           #else:
           b2=(y21-(lp_top_left*x21))
           # get x from lp_midpoint(x)+b1=lp_top_left(x)+b2
           # lp_midpoint(x)+b1=lp_top_left(x)+b2
           # lp_midpoint(x)-lp_top_left(x)=b2-b1
           # x(lp_midpoint-lp_top_left)=b2-b1
           #if (lp_midpoint==lp_top_left):
           #else:
           x=((b2-b1)/(lp_midpoint-lp_top_left))
           # get y from y=lp_midpoint(x)+b1
           y=((lp_midpoint*x)+b1)
           return x,y 
            
    def LineLength(self,ax,ay,bx,by):
           #a^2 + b^2 = c^2
           c_sq= ((ax-bx)**2) + ((ay-by)**2)
           c=self.Sqrt(c_sq)
           return c

    def Sqrt(self,xsq):
           x = abs((xsq)**(.5))
           return x
               
    def Arrow( self, layer, x1, y1, x2, y2, trans ):
           arrow_height=30
           arrow_width=10
           rise=abs(y2-y1)
           run=abs(x2-x1)
           line_angle=self.AngleFromSlope(rise, run)
           if (y2>y1):
               angle=line_angle
           else:
               angle=(line_angle - math.pi)
           perpendicular_angle=(-self.AngleFromSlope(run, rise))
           hx,hy = self.NewPointFromDistanceAndAngle(x1, y1, arrow_height, angle)
           w1x,w1y = self.NewPointFromDistanceAndAngle(x1, y1, arrow_width, perpendicular_angle)
           w2x,w2y = self.NewPointFromDistanceAndAngle(x1, y1,-arrow_width, perpendicular_angle)
           style = { 'fill':'green',
                     'stroke':'green',
                     'stroke-width':'8',
                     'stroke-linejoin':'miter',
                     'stroke-miterlimit':'4'} 
           my_path='M '+str(x1)+' '+str(y1)+' '+str(w1x)+' '+str(w1y)+' L '+str(hx)+' '+str(hy)+' L '+str(w2x)+' '+str(w2y)+' z'
           pathattribs = { inkex.addNS('label','inkscape') : 'Arrow',
                                               'transform' : trans,
                                                        'd': my_path, 
                                                    'style': simplestyle.formatStyle(style) }
           inkex.etree.SubElement( layer, inkex.addNS('path','svg'), pathattribs)

    def Grainline( self, parent, x1, y1, x2, y2, name, trans ):
           grain_path = 'M '+str(x1)+' '+str(y1)+' L '+str(x2)+' '+str(y2)
           self.Path( parent, grain_path, 'grainline', name, trans )
           self.Arrow( parent,x1,y1,x2,y2, trans )
           self.Arrow( parent,x2,y2,x1,y1, trans )

    def Buttons( self, mylayer, bx, by, button_number, button_distance, button_size ):
           buttonline='M '+ str(bx) +' '+ str(by) +' L '+ str(bx) +' '+ str( by + (button_number*button_distance) )  # vertical button line only! at this time...
           self.Path( mylayer, buttonline, 'fold', 'Button Line', '')
           i = 1
           y = by
           while i<=button_number :
              self.GetCircle( mylayer, bx, y, (button_size / 2), 'green', 'Button '+ str(i))
              buttonhole_path = 'M '+ str(bx) +' '+ str(y) +' L '+ str(bx-button_size) +' '+ str(y)
              self.Path( mylayer, buttonhole_path, 'green', 'Button Hole '+ str(i), '' )
              i = i + 1
              y = y + button_distance

    def Text(self, parent, x, y, font_size, label, string, trans ):

           style = {'text-align'     : 'center', 
                    'vertical-align' : 'top',
                    'text-anchor'    : 'middle', 
                    'font-size'      : str(font_size)+'px',
                    'fill-opacity'   : '1.0', 
                    'stroke'         : 'none',
                    'font-weight'    : 'normal', 
                    'font-style'     : 'normal', 
                    'fill'           : '#000000' 
                   }
           attribs = {'style'    : simplestyle.formatStyle( style ),
                     inkex.addNS( 'label', 'inkscape' ) : label,
                     'transform' : trans,
                     'x'         : str(x), 
                     'y'         : str(y)
                    }
           label         = inkex.etree.SubElement( parent, inkex.addNS( 'text', 'svg'), attribs)
           label.text    = string

           ###################### ###################### ###################### ###################### 

    def effect(self):

           ######################
           ### Get Parameters ### 
           ######################
           if ( self.options.measureunit == 'cm'):
               conversion = cm_to_pt
           else:
               conversion = in_to_pt

           height                     = self.options.height * conversion        #Pattern was written for height=5'9 or 176cm, 38" chest or 96cm
           chest                      = self.options.chest * conversion
           chest_length               = self.options.chest_length * conversion
           waist                      = self.options.waist * conversion
           back_waist_length          = self.options.back_waist_length * conversion                
           back_jacket_length         = self.options.back_jacket_length * conversion               
           back_shoulder_width        = self.options.back_shoulder_width * conversion              
           back_shoulder_length       = self.options.back_shoulder_length * conversion             
           back_underarm_width        = self.options.back_underarm_width * conversion
           back_underarm_length       = self.options.back_underarm_length * conversion             
           back_waist_to_hip_length   = self.options.back_waist_to_hip_length * conversion         
           nape_to_vneck              = self.options.nape_to_vneck * conversion
           sleeve_length              = self.options.sleeve_length * conversion

           neck_width           = chest/16 + (2*cm_to_pt)     # replace chest/16 with new parameter back_neck_width, jf_neck_width, neck_circumference
           back_shoulder_height = 2*cm_to_pt                  # replace 2*cm_to_pt with new parameter back_shoulder_height
           bp_width             = (back_shoulder_width* .5)   # back pattern width is relative to back_shoulder_width/2  (plus 1cm)
  
           ####################################
           ### Create reference & pattern layers ###
           ####################################
           reference_layer = self.GetNewLayer( self.document.getroot(), 'Reference' )        # reference_layer = reference information 
           pattern_layer   = self.GetNewLayer( self.document.getroot(), 'Steampunk Jacket' ) # pattern_layer = pattern lines & marks

           #################
           ### Signature ###
           #################
           my_layer = reference_layer
           x =  border
           y =  border
           self.Text( my_layer, 2*x, y, 60, 'Company', 'New Day Patterns', '' )
           self.Text( my_layer, 2*x, ( y + 65 ), 60, 'Pattern number', pattern_number, '' )
           begin_x, begin_y = x, ( y + 65*2 )
           next_pattern_startx, next_pattern_starty, next_pattern_start = self.GetDot( my_layer, begin_x, begin_y, 'next_pattern_start_1 - Begin Pattern Calculations Here' )

           ###################
           ### Jacket Back ###
           ###################
           my_layer = reference_layer
           # Top
           napex, napey, nape = self.GetDot( my_layer, next_pattern_startx, next_pattern_starty + ( nape_to_vneck * .25 ) + back_shoulder_height , 'nape' ) 
           back_pattern_startx, back_pattern_starty, back_pattern_start = self.GetDot( my_layer, napex, napey, 'back_pattern_start')
           back_pattern_endx, back_pattern_endy, back_pattern_end = self.GetDot( my_layer, back_pattern_startx + bp_width, back_pattern_starty, 'back_pattern_end')

           # Back Shoulder
           shoulder_startx, shoulder_starty, shoulder_start = self.GetDot( my_layer, back_pattern_startx, back_pattern_starty + back_shoulder_length, 'shoulder_start')
           back_shoulder_startx, back_shoulder_starty, back_shoulder_start = self.GetDot( my_layer, shoulder_startx, shoulder_starty, 'back_shoulder_start')
           back_shoulder_endx, back_shoulder_endy, back_shoulder_end = self.GetDot( my_layer, back_pattern_endx, back_shoulder_starty, 'back_shouder_end')
           back_shoulder_highx, back_shoulder_highy, back_shoulder_high = self.GetDot( my_layer, ( napex + neck_width ), napey - back_shoulder_height, 'back_shoulder_high')
           back_shoulder_lowx, back_shoulder_lowy, back_shoulder_low = self.GetDot( my_layer,( back_pattern_endx + (1*cm_to_pt) ), back_shoulder_starty, 'back_shoulder_low') 

           # Chest
           chest_startx, chest_starty, chest_start = self.GetDot( my_layer, napex, (napey + chest_length ),'chest_start')
           back_chest_startx, back_chest_starty, back_chest_start = self.GetDot( my_layer, chest_startx + (1*cm_to_pt), chest_starty, 'back_chest_start')
           back_chest_endx, back_chest_endy, back_chest_end = self.GetDot( my_layer, back_pattern_endx-(1*cm_to_pt), chest_starty, 'back_chest_end')

           # Waist
           waist_startx, waist_starty, waist_start = self.GetDot( my_layer, napex, napey + back_waist_length, 'waist_start')
           back_waist_startx, back_waist_starty, back_waist_start = self.GetDot( my_layer, waist_startx + (2.5*cm_to_pt), waist_starty, 'back_waist_start')
           back_waist_endx, back_waist_endy, back_waist_end = self.GetDot( my_layer, back_pattern_endx - (3*cm_to_pt), waist_starty, 'back_waist_end')

           # Hip 
           hip_startx, hip_starty, hip_start = self.GetDot( my_layer, napex, waist_starty + back_waist_to_hip_length, 'hip_start' )
           back_hip_startx, back_hip_starty, back_hip_start = self.GetDot( my_layer, hip_startx + (2*cm_to_pt), hip_starty, 'back_hip_start' )
           back_hip_endx, back_hip_endy, back_hip_end = self.GetDot( my_layer, back_pattern_endx -(2*cm_to_pt), hip_starty, 'back_hip_end' )  

           # Hem
           hem_startx, hem_starty, hem_start = self.GetDot( my_layer, napex, napey + back_jacket_length, 'hem_start')
           back_hem_startx, back_hem_starty, back_hem_start = self.GetDot( my_layer, hem_startx + (1.5*cm_to_pt), hem_starty, 'back_hem_start')
           back_hem_endx, back_hem_endy, back_hem_end = self.GetDot( my_layer, back_pattern_endx -(1.5*cm_to_pt), hem_starty, 'back_hem_end')
           Jacket_Back_Hem = 'M '+ back_hem_start +' L '+ back_hem_end

           # Hem Allowance 
           back_hem_allowance_startx, back_hem_allowance_starty, back_hem_allowance_start = self.GetDot(my_layer, back_hem_startx, back_hem_starty + hem_allowance, 'back_hem_allowance_start' )
           back_hem_allowance_endx, back_hem_allowance_endy, back_hem_allowance_end = self.GetDot( my_layer, back_hem_endx, back_hem_endy + hem_allowance, 'back_hem_allowance_end')
           Back_Hem_Allowance = 'L '+ back_hem_allowance_end +' L '+ back_hem_allowance_start   # moving right to left with path 

           ############################
           ### Back Reference Lines ###
           ############################
           # Pattern Start
           d = 'M '+ nape + ' L ' + hem_start
           self.Path(my_layer, d , 'reference' , 'Pattern Start Vertical Reference','')
           # Back Pattern Width
           d = 'M '+ back_pattern_end + ' L ' + str( back_pattern_endx) +' '+ str( hem_starty)
           self.Path(my_layer, d , 'reference' , 'Back Pattern Width Vertical Reference','') 
           # Back Shoulder Width
           d = 'M '+ back_shoulder_start +' L '+ back_shoulder_end 
           self.Path( my_layer, d, 'reference', 'Back Shoulder Width Horizontal Reference', '' )
           # Back Shoulder Heigth
           d = 'M '+ back_shoulder_high +' v '+ str(back_shoulder_height)
           self.Path( my_layer, d, 'reference', 'Back Shoulder Height Vertical Reference', '' )
         
           #############################
           ### Back Reference Points ###
           #############################
           # Back Sleeve Balance Point
           sleeve_back_balance_pointx, sleeve_back_balance_pointy, sleeve_back_balance_point=self.GetDot( my_layer, back_pattern_endx, chest_starty - (12*cm_to_pt), 'sleeve_back_balance_point')     
           # Back Underarm point
           back_underarm_pointx, back_underarm_pointy, back_underarm_point = self.GetDot( my_layer, back_pattern_endx, chest_starty - (6*cm_to_pt), 'back_underarm_point')

           #######################
           ### Back Seam Lines ###
           #######################
           # Back Center seam line: clockwise from bottom left: hem up to nape
           x1, y1       = self.XYwithSlope( back_hip_startx, back_hip_starty, back_hem_startx, back_hem_starty, ( abs( back_hip_starty - back_waist_starty ) * (.5) ) , 'normal' )
           c1x, c1y, c1 = self.GetDot( my_layer, x1, y1, 'c1' ) 
           c2x, c2y, c2 = self.GetDot( my_layer, back_waist_startx, ( back_waist_starty + ( abs( back_waist_starty - back_hip_starty ) * (.7) ) ) , 'c2' )
           c3x, c3y, c3 = self.GetDot( my_layer, back_waist_startx, back_waist_starty - ( abs( back_waist_starty - back_chest_starty ) * (.3) ) , 'c3' )
           x1, y1       = self.XYwithSlope( back_chest_startx, back_chest_starty, back_shoulder_startx, back_shoulder_starty, ( abs( back_waist_starty - back_chest_starty ) * (.5) ) , 'normal' )
           c4x, c4y, c4 = self.GetDot( my_layer, x1, y1, 'c4' )
           c5x, c5y, c5 = self.GetDot( my_layer, back_chest_startx - ( abs( back_chest_startx - back_shoulder_startx) * (.4) ) , back_chest_starty - ( abs( back_chest_starty - back_shoulder_starty ) * (.2) ), 'c5' )
           c6x, c6y, c6 = self.GetDot( my_layer, back_shoulder_startx, back_chest_starty - ( abs( back_chest_starty - back_shoulder_starty ) * (.9) ) , 'c6' )
           Back_Center  = 'M '+ back_hem_start +' L '+ back_hip_start +' C '+ c1 +' '+ c2 +' '+ back_waist_start +' C '+ c3 +' '+ c4 +' '+ back_chest_start +' C '+ c5 +' '+ c6+','+ back_shoulder_start +' L '+ nape

           # Back Neck seam line: clockwise from nape to high point of shoulder
           my_length1   = ( abs( back_shoulder_highy - napey )  * (.75)  )
           x1, y1       = self.XYwithSlope( back_shoulder_highx, back_shoulder_highy, back_shoulder_lowx, back_shoulder_lowy, my_length1, 'perpendicular')
           c1x, c1y, c1 = self.GetDot( my_layer, x1, y1, 'c1') #c1 control point should be on line from shoulder_high, perpendicular to shoulder line
           my_length2   = ( -(abs( back_shoulder_highx - napex ) ) * (.50) )    
           x1, y1       = self.XYwithSlope( napex, napey, back_shoulder_highx, napey, my_length2, 'normal')
           c2x, c2y, c2 = self.GetDot( my_layer, x1, y1, 'c2')
           Back_Neck    = ' C '+ c2 +' '+ c1 +' '+ back_shoulder_high

           # Back Shoulder seam line: clockwise from high point of shoulder to low point of shoulder
           c1x, c1y, c1  = self.GetDot( my_layer, back_shoulder_highx + ( abs( back_shoulder_lowx - back_shoulder_highx ) * (.33) ) , back_shoulder_highy + ( abs( back_shoulder_lowy - back_shoulder_highy ) * (.4) ), 'c1' )
           c2x, c2y, c2  = self.GetDot( my_layer, back_shoulder_highx + ( abs( back_shoulder_lowx - back_shoulder_highx ) * (.6) ), back_shoulder_highy + ( abs( back_shoulder_lowy - back_shoulder_highy ) * (.66) ), 'c2' ) #
           Back_Shoulder = ' C '+ c1 +' '+ c2 +' '+ back_shoulder_low
           # Back Armhole seam line: clockwise from low point of shoulder to back underarm 
           Back_Armhole  = ' Q '+ sleeve_back_balance_point +' '+ back_underarm_point
           # Back Side seam line: clockwise from back underarm down to hem
           x1, y1       = self.XYwithSlope( back_chest_endx, back_chest_endy, back_underarm_pointx, back_underarm_pointy, abs(back_chest_starty - back_waist_endy) * (.5) , 'normal')
           c1x, c1y, c1 = self.GetDot( my_layer, x1, y1, 'side_back_chest_to_back_waist_c1' )
           c2x, c2y, c2 = self.GetDot( my_layer, back_waist_endx, ( back_waist_endy - ( abs( back_waist_endy - back_chest_endy ) * (.3) ) ), 'side_back_chest_to_back_waist_c2') 
           c3x, c3y, c3 = self.GetDot( my_layer, back_waist_endx, ( back_waist_endy + ( abs( back_waist_endy - back_hem_endy ) * (.3) ) ), 'side_back_waist_to_back_hem_c3' )
           x1, y1       = self.XYwithSlope( back_hip_endx, back_hip_endy, back_hem_endx, back_hem_endy, ( abs(back_hip_endy - back_waist_endy) * (.5) ), 'normal')
           c4x, c4y, c4 = self.GetDot( my_layer, x1, y1, 'side_back_waist_to_back_hem_c4' )
           Back_Side    = ' L '+ back_chest_end +' C '+ c1+ ' '+ c2 +' '+ back_waist_end +' C '+ c3 +' '+ c4 +' '+ back_hip_end +' L '+ back_hem_end       
           # Grainline 
           G1x, G1y, G1 = self.GetDot( my_layer, back_shoulder_highx, back_underarm_pointy, 'G1' )
           G2x, G2y, G2 = self.GetDot( my_layer, G1x, G1y + (40*cm_to_pt), 'G2' )

           # Jacket Back Pattern path
           Jacket_Back_Pattern = Back_Center +' '+ Back_Neck + ' '+ Back_Shoulder +' '+ Back_Armhole +' '+ Back_Side +' '+ Back_Hem_Allowance +' z'

           ########################
           ### Draw Jacket Back ###
           ########################
           my_layer = self.GetNewLayer( pattern_layer, 'Jacket Back')
           self.Path( my_layer, Jacket_Back_Hem, 'fold', 'Jacket Back Hemline', '' )
           self.Path( my_layer, Jacket_Back_Pattern, 'seam', 'Jacket Back Seamline', '')
           self.Path( my_layer, Jacket_Back_Pattern, 'pattern', 'Jacket Back Cuttingline', '')
           self.Grainline( my_layer, G1x, G1y, G2x, G2y, 'Jacket Back Grainline', '' )
           self.Text( my_layer, back_shoulder_highx, shoulder_starty, 40, 'Company', 'New Day Patterns', '' )
           self.Text( my_layer, back_shoulder_highx, shoulder_starty + 45, 40, 'Pattern number', pattern_number, '' )
           self.Text( my_layer, back_shoulder_highx, shoulder_starty + 2*45, 40, 'Pattern name', 'Jacket Back - A', '' )
           self.Text( my_layer, back_shoulder_highx, shoulder_starty + 3*45, 40, 'Cut', 'Cut 2', '' )
           next_pattern_startx, next_pattern_starty, next_pattern_start = self.GetDot( reference_layer, back_pattern_endx + pattern_offset, napey, 'next_pattern_start 2' )
           lowest_x = begin_x

           ####################
           ### Front Jacket ###
           ####################
           my_layer = reference_layer

           # constants
           jf_armhole_point_offset    = ( 5.5*cm_to_pt )
           jf_chest_start_offset      = ( 4.5*cm_to_pt )
           jf_waist_start_offset      = ( 4.5*cm_to_pt )
           jf_hip_start_offset        = ( 1.5*cm_to_pt )
           jf_hem_start_offset        = ( 0*cm_to_pt )

           chest_scale                   = ( chest*0.5 )                           #scale (width) of half the pattern is chest/2, 

           jf_chest_center_offset     = ( chest_scale / 2 ) - ( 3.5*cm_to_pt )  # half chest_scale - 3.5cm
           jf_chest_underarm_offset   = ( 5.5*cm_to_pt )
           jf_armscye_width_offset    = ( chest_scale / 4 ) + ( 2*cm_to_pt )    # one_fourth chest_scale + 2cm
           jf_pattern_end_offset      = ( 2*cm_to_pt )
           jf_button_offset           = ( 2*cm_to_pt )                          # same as jf_pattern_end_offset
 
           back_shoulder_ease            = ( 1*cm_to_pt )
           jf_shoulder_adjustment     = ( 1*cm_to_pt )
           jf_shoulder_middle_offset  = ( 1.3*cm_to_pt )
           jf_shoulder_length         = ( self.LineLength( back_shoulder_lowx, back_shoulder_lowy, back_shoulder_highx, back_shoulder_highy ) - jf_shoulder_adjustment  )
           jf_shoulder_high_offset     = ( chest_scale / 8 ) + jf_shoulder_adjustment    # one-eigth scale + 1cm

           jf_armhole_depth_1         = ( 4*cm_to_pt )
           jf_armhole_depth_2         = ( 2*cm_to_pt )
           jf_armhole_depth_3         = ( 5*cm_to_pt )
           jf_armhole_depth_4         = ( 2*cm_to_pt )
           jf_armhole_curve_3x_offset = ( 0.5*cm_to_pt )
           jf_armhole_curve_3y_offset = ( 3.5*cm_to_pt )

           jf_hem_offset                 = ( 6.5*cm_to_pt )
           jf_hem_curve_reference_offset = ( 2.5*cm_to_pt )

           jf_side_dart_width_1             = ( 1*cm_to_pt )
           jf_side_dart_width_2             = ( 1*cm_to_pt )
           jf_side_dart_widest_point_offset = ( 2*cm_to_pt )


           jf_neck_height       = ( 6.5*cm_to_pt )
           jf_neck_curve_offset = ( 2.5*cm_to_pt )

           jf_lapel_height      = ( 16.5*cm_to_pt )
           jf_lapel_dart_width  = ( 1.3*cm_to_pt )
           jf_lapel_dart_height = ( 9*cm_to_pt )
           jf_lapel_dart_offset = ( 2.5*cm_to_pt )

           up_width         = ( 10*cm_to_pt )
           up_width_offset  = ( 3.7*cm_to_pt )
           up_height        = ( 2*cm_to_pt )
           up_height_offset = ( 3*cm_to_pt )

           lp_width            = ( 15*cm_to_pt )
           lp_height           = ( 5.5*cm_to_pt ) 
           lp_flap_height      = ( 1.3*cm_to_pt )    # extension required to sew pocket into Jacket         
           lp_slant_offset     = ( 1*cm_to_pt )      # x offset to make pocket diagonal
           lp_placement_offset = ( 28*cm_to_pt )

           # reference points
           jf_pattern_startx, jf_pattern_starty, jf_pattern_start = self.GetDot( my_layer, next_pattern_startx, next_pattern_starty, 'jf_pattern_start' )
           jf_pattern_endx, jf_pattern_endy, jf_pattern_end = self.GetDot( my_layer, jf_pattern_startx + jf_chest_start_offset + jf_armscye_width_offset + jf_chest_center_offset + jf_pattern_end_offset, napey, 'jf_pattern_end' )   
           # later -> add diff between jf_pattern and value related to max(chest,waist,hip), then calculate everything by subtraction from jf_pattern_end
           jf_centerx, jf_centery, jf_center = self.GetDot( my_layer, jf_pattern_endx - jf_pattern_end_offset, jf_pattern_endy, 'jf_center' )
           jf_chest_startx, jf_chest_starty, jf_chest_start = self.GetDot( my_layer, jf_pattern_startx + jf_chest_start_offset , chest_starty, 'jf_chest_start' )
           jf_armscye_widthx, jf_armscye_widthy, jf_armscye_width = self.GetDot( my_layer, jf_chest_startx + jf_armscye_width_offset, napey, 'jf_armscye_width' ) 
           jf_chest_endx, jf_chest_endy, jf_chest_end = self.GetDot( my_layer, jf_pattern_endx , chest_starty, 'jf_chest_end' )
           jf_waist_startx, jf_waist_starty, jf_waist_start = self.GetDot( my_layer, jf_pattern_startx + jf_waist_start_offset, waist_starty, 'jf_waist_start' )
           jf_waist_endx, jf_waist_endy, jf_waist_end = self.GetDot( my_layer, jf_pattern_endx , waist_starty, 'jf_waist_end' ) 
           jf_hip_startx, jf_hip_starty, jf_hip_start = self.GetDot( my_layer, jf_pattern_startx + jf_hip_start_offset, hip_starty, 'jf_hip_start' ) 
           jf_hip_endx, jf_hip_endy, jf_hip_end = self.GetDot( my_layer, jf_pattern_endx , hip_starty, 'jf_hip_end' )
           jf_hem_startx, jf_hem_starty, jf_hem_start = self.GetDot( my_layer, jf_pattern_startx + jf_hem_start_offset, hem_starty, 'jf_hem_start' )
           jf_hem_endx, jf_hem_endy, jf_hem_end = self.GetDot( my_layer, jf_pattern_endx , hem_starty, 'jf_hem_end' )
           jf_curve_startx, jf_curve_starty, jf_curve_start = self.GetDot( my_layer, jf_pattern_endx, waist_starty + ( abs( waist_starty - hip_starty ) * (.5) ), 'jf_curve_start')
           jf_hem_curve_reference_endx, jf_hem_curve_reference_endy, jf_hem_curve_reference_end = self.GetDot( my_layer, jf_pattern_endx, jf_hem_endy+ jf_hem_curve_reference_offset , 'jf_hem_curve_reference_end')
           #pattern_offset_2x, pattern_offset_2y, pattern_offset_2 = self.GetDot( my_layer, jf_pattern_endx + pattern_offset, back_pattern_starty, 'pattern_offset_2' )
           pattern_begin_row_2x, pattern_begin_row_2y, pattern_begin_row_2 = self.GetDot( my_layer, begin_x, jf_hem_curve_reference_endy + pattern_offset, 'pattern_begin_row_2' )

           # important points along chest reference line
           jf_chest_underarmx, jf_chest_underarmy, jf_chest_underarm = self.GetDot( my_layer, jf_armscye_widthx, chest_starty, 'jf_chest_underarm' )
           jf_dart_5x, jf_dart_5y, jf_dart_5 = self.GetDot( my_layer, jf_chest_underarmx - jf_chest_underarm_offset, chest_starty, 'jf_dart_5' )
           jf_dart_1x, jf_dart_1y, jf_dart_1 = self.GetDot( my_layer, jf_dart_5x - (1*cm_to_pt), chest_starty, 'jf_dart_1' )
           jf_button_topx , jf_button_topy, jf_button_top = self.GetDot( my_layer, jf_centerx, chest_starty, 'jf_button_top' )  
           jf_armhole_pointx, jf_armhole_pointy, jf_armhole_point = self.GetDot( my_layer, jf_pattern_startx + jf_armhole_point_offset, back_underarm_pointy, 'jf_armhole_point' )   

           # horizontal reference lines
           d = 'M '+ nape + ' L ' + jf_pattern_end
           self.Path( my_layer, d, 'reference', 'Front Top Reference', '' )           
           d = 'M '+ chest_start + ' L ' + jf_chest_end
           self.Path( my_layer, d, 'reference', 'Front Chest Reference', '' )           
           d = 'M '+ waist_start + ' L ' + jf_waist_end
           self.Path( my_layer, d, 'reference', 'Front Waist Reference', '' )
           d = 'M '+ hem_start + ' L ' + jf_hem_end
           self.Path( my_layer, d, 'reference', 'Front Hip Reference', '' )
           d = 'M '+ hip_start + ' L ' + jf_hip_end
           self.Path( my_layer, d, 'reference', 'Front Hem Reference', '' )
           d = 'M '+ jf_hem_start +' L '+ jf_hem_curve_reference_end
           self.Path( my_layer, d, 'reference', 'Front Curved Hem Reference', '' )

           # vertical reference lines
           #d = 'M '+ jf_pattern_start + ' L ' + str(jf_pattern_startx) +' '+ str(hem_starty)
           d = 'M '+ jf_pattern_start + ' V ' + str(hem_starty)
           self.Path( my_layer, d, 'reference', 'Front Start Reference', '' )
           d = 'M '+ jf_pattern_end + ' L ' + jf_hem_end
           self.Path( my_layer, d, 'reference', 'Front End Reference', '' )
           d = 'M '+ jf_center + ' V ' + str(hem_starty)
           self.Path( my_layer, d, 'reference', 'Front Center Reference', '' )
           d = 'M '+ jf_armscye_width + ' V ' + str(hem_starty)
           self.Path( my_layer, d, 'reference', 'Front Armscye Reference', '' )
           d = 'M '+ jf_pattern_end + ' V ' + str(hem_starty)
           self.Path( my_layer, d, 'reference', 'jf_pattern_end Reference Line', '' )

           # Front Side Seam --> bottom to top, from left side hem to armhole point
           x1, y1 = self.XYwithSlope( jf_hip_startx, jf_hip_starty, jf_hem_startx, jf_hem_starty, abs(jf_hip_starty-jf_waist_starty)*(.3) , 'normal' )
           c1x, c1y, c1 = self.GetDot( my_layer, x1, y1, 'c1')
           c2x, c2y, c2 = self.GetDot( my_layer, jf_waist_startx, jf_waist_starty + ( abs(jf_waist_starty-jf_hip_starty) * (.3) ), 'c2' )
           c3x, c3y, c3 = self.GetDot( my_layer, jf_waist_startx, jf_waist_starty - ( abs(jf_waist_starty-jf_chest_starty) * (.3) ), 'c3' )
           x1, y1 = self.XYwithSlope( jf_chest_startx, jf_chest_starty, jf_armhole_pointx, jf_armhole_pointy, ( abs(jf_waist_starty-jf_chest_starty) * (.3) ), 'normal' )
           c4x, c4y, c4 = self.GetDot( my_layer, x1, y1, 'c4' )
           c5x, c5y, c5 = self.GetDot( my_layer, jf_chest_startx + ( abs(jf_chest_startx - jf_armhole_pointx) * (.2) ), jf_chest_starty - ( abs(jf_chest_starty - jf_armhole_pointy) * (.3) ), 'c5' )
           JF_Side = 'M '+ jf_hem_start +' L '+ jf_hip_start +' C '+ c1 +' '+ c2 +' '+ jf_waist_start +' C '+ c3 +' '+ c4 +' '+ jf_chest_start +' Q '+ c5 +' '+ jf_armhole_point

           # Front Shoulder Seam --> left to right, from shoulder low to  shoulder high
           jf_shoulder_highx, jf_shoulder_highy, jf_shoulder_high = self.GetDot( my_layer, jf_armscye_widthx + jf_shoulder_high_offset, napey, 'jf_shoulder_high') 
           x1, y1 = self.XYwithSlope( jf_shoulder_highx, jf_shoulder_highy, jf_armscye_widthx, jf_armscye_widthy + jf_shoulder_middle_offset, -jf_shoulder_length, 'normal' )
           jf_shoulder_lowx, jf_shoulder_lowy, jf_shoulder_low = self.GetDot( my_layer, x1, y1, 'jf_shoulder_low' )
           c1x, c1y, c1 = self.GetDot( my_layer, jf_shoulder_highx - abs( jf_shoulder_lowx - jf_shoulder_highx ) * (.85), jf_shoulder_highy + abs( jf_shoulder_lowy - jf_shoulder_highy ) * (.7), 'c1' )           
           c2x, c2y, c2 = self.GetDot( my_layer, jf_shoulder_highx - abs( jf_shoulder_lowx - jf_shoulder_highx ) * (.45), jf_shoulder_highy + abs( jf_shoulder_lowy - jf_shoulder_highy ) * (.15), 'c2' )
           #my_path='M '+jf_shoulder_low+' C '+c1+' '+c2+' '+jf_shoulder_high
           JF_Shoulder = ' C '+ c1 +' '+ c2 +' '+ jf_shoulder_high

           # Armhole/Armscye curve,  part 1 --> left to right, from armhole point to side dart 
           x, y = self.XYwithSlope( jf_chest_startx, jf_chest_starty, jf_chest_startx-100, jf_chest_starty+100, jf_armhole_depth_1, 'normal' ) # 4cm at 45degree angle
           jf_armhole_curve_1x, jf_armhole_curve_1y, jf_armhole_curve_1 = self.GetDot( my_layer, x, y, 'jf_armhole_curve_1' )
           c1x, c1y, c1 = self.GetDot( my_layer, jf_dart_1x - ( abs( jf_dart_1x - jf_armhole_pointx ) * (.5) ), jf_dart_1y, 'c1' )
           c2x, c2y, c2 = self.GetDot( my_layer, jf_dart_1x - ( abs( jf_dart_1x - jf_armhole_pointx ) * (.9) ), jf_dart_1y - ( abs( jf_dart_1y - jf_armhole_pointy ) * (.8) ), 'c2' )
           d = 'M '+ jf_chest_start +' L '+ jf_armhole_curve_1
           self.Path( my_layer, d, 'reference', 'jf_armhole_depth_1 Reference Line', '' )
           JF_Armscye_1 = ' C '+ c2 +' '+ c1 +' '+ jf_dart_1
          
           # Armhole/Armscye curve,  part 2 --> left to right, from side dart to shoulder low
           x, y = self.XYwithSlope( jf_chest_underarmx, jf_chest_underarmy, jf_chest_underarmx + 100, jf_chest_underarmy + 100, jf_armhole_depth_2, 'normal') # 2cm at 135degree angle  
           jf_armhole_curve_2x, jf_armhole_curve_2y, jf_armhole_curve_2 = self.GetDot( my_layer, x, y, 'jf_armhole_curve_2' )
           jf_armhole_curve_2bx, jf_armhole_curve_2by, jf_armhole_curve_2b = self.GetDot( my_layer, jf_chest_underarmx, jf_chest_underarmy - jf_armhole_depth_2, 'jf_armhole_curve_2b' )  # 2cm at 90degree angle (vertical)
           mid_point = ( self.LineLength( jf_shoulder_lowx, jf_shoulder_lowy, jf_armhole_curve_2bx, jf_armhole_curve_2by ) * (0.5) )
           x1 , y1   = self.XYwithSlope( jf_armhole_curve_2bx, jf_armhole_curve_2by, jf_shoulder_lowx, jf_shoulder_lowy, -mid_point, 'normal' )
           jf_armhole_curve_2b_midptx, jf_armhole_curve_2b_midpty, jf_armhole_curve_2b_midpt = self.GetDot( my_layer, x1, y1, 'jf_armhole_curve_2b_midpt' )
           jf_armhole_curve_3x, jf_armhole_curve_3y, jf_armhole_curve_3 = self.GetDot( my_layer, jf_chest_underarmx, jf_chest_underarmy - jf_armhole_depth_3, 'jf_armhole_curve_3' )
           x1, y1 = self.XYwithSlope( jf_armhole_curve_2b_midptx, jf_armhole_curve_2b_midpty, jf_shoulder_lowx, jf_shoulder_lowy, jf_armhole_depth_4, 'perpendicular' ) 
           jf_armhole_curve4x, jf_armhole_curve_4y, jf_armhole_curve_4 = self.GetDot( my_layer, x1, y1, 'jf_armhole_curve_4' )
           c3x, c3y, c3 = self.GetDot( my_layer, jf_dart_5x + ( abs( jf_dart_5x - jf_armhole_curve_3x )* (.364) ),  jf_dart_5y + ( abs( jf_dart_5y - jf_armhole_curve_3y ) * (.084) ), 'c3' ) 
           c4x, c4y, c4 = self.GetDot( my_layer, jf_armhole_curve_3x + ( abs( jf_dart_5x - jf_armhole_curve_3x )  * (0.182) ) ,jf_armhole_curve_3y + ( abs( jf_dart_5y - jf_armhole_curve_3y ) * (0.8) ), 'c4' )
           c5x, c5y, c5 = self.GetDot( my_layer, jf_armhole_curve_3x - ( abs( jf_armhole_curve_3x - jf_shoulder_lowx ) * (0.1) ), jf_armhole_curve_3y - ( abs( jf_armhole_curve_3y - jf_shoulder_lowy ) * (.4) ), 'c5' )
           c6x, c6y, c6 = self.GetDot( my_layer, jf_shoulder_lowx + ( abs( jf_armhole_curve_3x - jf_shoulder_lowx ) * (.2) ), jf_shoulder_lowy + ( abs( jf_armhole_curve_3y - jf_shoulder_lowy ) * (.12) ), 'c6' )
           d = 'M '+ jf_chest_underarm +' L '+ jf_armhole_curve_2
           self.Path( my_layer, d, 'reference', 'jf_armhole_depth_2 Reference Line', '' )
           d = 'M '+ jf_shoulder_low +' L '+ jf_armhole_curve_2b
           self.Path( my_layer, d, 'reference', 'jf_armhole_depth_2-b Reference Line', '' )
           d = 'M '+ jf_armhole_curve_2b_midpt +' L '+ jf_armhole_curve_4
           self.Path( my_layer, d, 'reference', 'jf_armhole_curve_4 Reference Line','')
           JF_Armscye_2 = 'L '+ jf_dart_5 +' C '+ c3+ ' ' + c4 +' '+ jf_armhole_curve_3 + ' C '+ c5 +' '+ c6 +' '+ jf_shoulder_low

           # neck curve & lapel roll line--> left to right, from high shoulder to chest line/top button
           jf_lapel_pointx, jf_lapel_pointy, jf_lapel_point = self.GetDot( my_layer, jf_chest_endx, jf_chest_endy - jf_lapel_height, 'jf_lapel_point' )
           neck_ref_pointx, neck_ref_pointy, neck_ref_point = self.GetDot( my_layer, jf_shoulder_highx, jf_shoulder_highy + jf_neck_height, 'neck_ref_point' )
           x1, y1 = self.XYwithSlope( neck_ref_pointx, neck_ref_pointy, neck_ref_pointx - 100, neck_ref_pointy + 100, jf_neck_curve_offset, 'normal' )
           neck_curve_1x, neck_curve_1y, neck_curve_1 = self.GetDot( my_layer, x1, y1, 'neck_curve_1' )
           x1, y1 = self.XY( jf_shoulder_highx, jf_shoulder_highy, jf_shoulder_lowx, jf_shoulder_lowy, jf_neck_curve_offset )   # neck curve offset = lapel offset extended from high shoulder
           jf_lapel_reference_pointx, jf_lapel_reference_pointy, jf_lapel_reference_point = self.GetDot( my_layer, x1, y1, 'jf_lapel_reference_point' )
           x1, y1 = self.Intersect( neck_ref_pointx, neck_ref_pointy, jf_lapel_pointx, jf_lapel_pointy, jf_lapel_reference_pointx, jf_lapel_reference_pointy, jf_chest_endx, jf_chest_endy )
           jf_lapel_neck_intersectx, jf_lapel_neck_intersecty, jf_lapel_neck_intersect = self.GetDot( my_layer, x1, y1, 'jf_lapel_neck_intersecty' )
           d = 'M '+ neck_ref_point + ' L '+ neck_curve_1
           self.Path( my_layer, d, 'reference', 'neck_curve_1 reference line', '' )
           Lapel_Roll_Line = 'M '+ jf_lapel_point +' L '+ neck_ref_point +' L '+ jf_shoulder_high +' '+ jf_lapel_reference_point +' '+ jf_chest_end  # lapel ends at chest ref line

           # lapel dart 
           jf_lapel_dart_midpointx, jf_lapel_dart_midpointy, jf_lapel_dart_midpoint = self.GetDot( my_layer, jf_lapel_pointx - ( ( jf_lapel_pointx - jf_lapel_neck_intersectx ) * (.5) ), jf_lapel_pointy, 'jf_lapel_dart_midpoint' ) 
           jf_lapel_dart_1x, jf_lapel_dart_1y ,jf_lapel_dart_1 = self.GetDot( my_layer, jf_lapel_dart_midpointx + ( jf_lapel_dart_width * (.5) ), jf_lapel_pointy, 'jf_lapel_dart_1' ) 
           jf_lapel_dart_2x, jf_lapel_dart_2y, jf_lapel_dart_2 = self.GetDot( my_layer, jf_lapel_neck_intersectx + ( abs( jf_lapel_neck_intersectx - jf_lapel_dart_midpointx) * (.5)  ), jf_lapel_pointy + ( abs( jf_lapel_pointy - jf_chest_endy ) * (.5) ), 'jf_lapel_dart_2' )
           jf_lapel_dart_3x, jf_lapel_dart_3y, jf_lapel_dart_3 = self.GetDot( my_layer, jf_lapel_dart_midpointx - ( jf_lapel_dart_width * (.5) ), jf_lapel_pointy, 'jf_lapel_dart_3' ) 
           Lapel_Dart = 'M '+ jf_lapel_dart_1 +' L '+ jf_lapel_dart_2 +' L '+ jf_lapel_dart_3

           # Side Dart
           lp_midpointx, lp_midpointy, lp_midpoint = self.GetDot( my_layer, jf_chest_underarmx, jf_chest_underarmy + lp_placement_offset, 'lp_midpoint' )
           m = self.Slope( jf_hem_startx, jf_hem_starty, jf_hem_curve_reference_endx, jf_hem_curve_reference_endy, 'normal' ) # lower pocket is parallel to slanted hem edge
           b = lp_midpointy - ( m * lp_midpointx )
           lp_top_leftx, lp_top_lefty, lp_top_left = self.GetDot( my_layer, lp_midpointx - ( lp_width * (.5) ),  m *  ( lp_midpointx -  ( lp_width  * (.5) ) )  + b, 'lp_top_left' )
           lp_top_rightx, lp_top_righty, lp_top_right = self.GetDot( my_layer, lp_midpointx + ( lp_width * (.5) ),  m *  ( lp_midpointx +  ( lp_width  * (.5) ) )  + b, 'lp_top_left' )          
           x1, y1 = self.XY( jf_chest_underarmx, jf_chest_underarmy + lp_placement_offset, lp_top_leftx, lp_top_lefty, -( 4*cm_to_pt ))
           O1x,O1y,O1=self.GetDot(my_layer,x1,y1,'O1')
           O2x,O2y,O2=self.GetDot(my_layer,jf_dart_1x+(abs(jf_dart_5x-jf_dart_1x)/2),jf_dart_1y,'O2')
           x,y=self.XY(O2x,O2y,O1x,O1y,seam_allowance)
           O2ax,O2ay,O2a=self.GetDot(my_layer,x,y,'O2a')
           my_path='M '+O1+' L '+O2
           self.Path(my_layer,my_path,'reference','Side Dart Reference Line','')
           m=(O1y-O2y)/(O1x-O2x)
           b=O1y-(O1x*m)
           y1=jf_waist_starty-(2*cm_to_pt)
           O3x,O3y,O3=self.GetDot(my_layer,(y1-b)/m,y1,'O3')
           O4x,O4y,O4=self.GetDot(my_layer,O3x-(1*cm_to_pt),O3y,'O4')
           O5x,O5y,O5=self.GetDot(my_layer,O3x+(1*cm_to_pt),O3y,'O5')
           c1x,c1y,c1=self.GetDot(my_layer,O4x-30,jf_dart_1y+(abs(jf_dart_1y-O4y))*(.80),'c1')     #sidedartcontroup_bottom_left,2, etc - c1, c2
           c2x,c2y,c2=self.GetDot(my_layer,O4x,O4y+(abs(O4y-O1y))*(.20),'c2')
           c3x,c3y,c3=self.GetDot(my_layer,O5x+20,jf_dart_5y+(abs(jf_dart_5y-O5y))*(.85),'c3')
           c4x,c4y,c4=self.GetDot(my_layer,O5x,O5y+(abs(O5y-O1y))*(.20),'c4')
           x,y=self.XY(jf_dart_1x,jf_dart_1y,c1x,c1y,seam_allowance)
           jf_dart_1ax,jf_dart_1ay,jf_dart_1a=self.GetDot(my_layer,x,y,'jf_dart_1a')
           x,y=self.XY(jf_dart_5x,jf_dart_5y,c3x,c3y,seam_allowance)
           jf_dart_5ax,jf_dart_5ay,jf_dart_5a=self.GetDot(my_layer,x,y,'jf_dart_5a')        
           JF_Side_Dart='M '+jf_dart_1a+' L '+jf_dart_1+' C '+c1+' '+c2+' '+ O1+' C '+c4+' '+c3+' '+jf_dart_5+' L '+jf_dart_5a
           x1,y1=O4x-1.5*cm_to_pt,O4y
           x2,y2=O5x+1.5*cm_to_pt,O5y
           JF_Side_Dart_Foldline='M '+O1+' L '+O2a +' M '+ str(x1)+' '+str(y1)+' L '+str(x2)+' '+str(y2)
    
           # Shoulder to hem --> Clockwise from high shoulder point, around neck curve, to lapel point, down gentle curve of lapel to level of top button, straight down to beginning of front jacket curve, continuing clockwise to end of front jacket curve.
           m = self.Slope( jf_hem_startx, jf_hem_starty, jf_hem_curve_reference_endx, jf_hem_curve_reference_endy, 'normal' )
           b = jf_hem_starty - ( m * jf_hem_startx )
           jf_curve_endx, jf_curve_endy, jf_curve_end = self.GetDot( my_layer, jf_lapel_reference_pointx, ( ( m * jf_lapel_reference_pointx ) + b ), 'jf_curve_end' )
           control_length = (  self.LineLength( jf_shoulder_highx, jf_shoulder_highy, jf_lapel_neck_intersectx, jf_lapel_neck_intersecty ) * ( 0.33 ) )
           c1x, c1y, c1 = self.GetDot( my_layer, jf_shoulder_highx, ( jf_shoulder_highy + control_length ), 'c1' ) #c1-neck curve between jf_shoulder_high & jf_lapel_neck_intersect
           c2x, c2y, c2 = self.GetDot( my_layer, ( jf_lapel_neck_intersectx - control_length ), jf_lapel_neck_intersecty, 'c2') #c2-neck curve between jf_shoulder_high & jf_lapel_neck_intersect
           c3x, c3y, c3 = self.GetDot( my_layer, jf_lapel_pointx + 1*cm_to_pt, jf_lapel_pointy + ( abs( jf_lapel_pointy - jf_pattern_endy ) * 0.5 ), 'c3' ) #c3-curved lapel edgebetween jf_lapel_point and jf_buttonhole_top 
           c4x, c4y, c4 = self.GetDot( my_layer, jf_pattern_endx, hip_starty, 'c4' ) # c4 for curve at front hem --> b/w jf_curve_start and jf_curve_end
           c5x, c5y, c5 = self.GetDot( my_layer, jf_pattern_endx, hem_starty, 'c5' ) # c5 for curve at front hem --> b/w jf_curve_start and jf_curve_end
           #my_path='M '+jf_shoulder_high+' C '+c1+' '+c2+' '+jf_lapel_neck_intersect+' L '+jf_lapel_point+' Q '+c1+' '+jf_chest_end+' '+jf_curve_start+' C '+c4+' '+c5+ ' '+jf_curve_end+' L '+jf_hem_start
           JF_Shoulder_to_Hem = ' C '+c1+' '+c2+' '+jf_lapel_neck_intersect+' L '+jf_lapel_point+' Q '+c3+' '+jf_chest_end+' L '+jf_curve_start+' C '+ c4 +' '+c5+ ' '+jf_curve_end

           # Buttons and Buttonholes        
           Button_x        = jf_button_topx
           Button_y        = jf_button_topy
           Button_number   = 4
           Button_distance = ( abs( jf_button_topy - waist_starty ) / 2 )
           Button_size     = ( (.75) * in_to_pt)

           #Hem Line & Allowance
           JF_Hem_Line           = ' M ' + jf_hem_start + ' L ' + jf_curve_end
           jf_hem_allow_1x, jf_hem_allow_1y, jf_hem_allow_1 = self.GetDot( my_layer, jf_hem_startx, jf_hem_starty + hem_allowance, 'hem_allow_1' )
           jf_hem_allow_2x, jf_hem_allow_2y, jf_hem_allow_2 = self.GetDot( my_layer, jf_curve_endx, jf_curve_endy + hem_allowance, 'hem_allow_2' )
           JF_Hem_Allowance = ' L ' + jf_hem_allow_2 + ' ' + jf_hem_allow_1
     
           # Grainline
           jf_g1x, jf_g1y, jf_g1 = self.GetDot( my_layer, jf_chest_underarmx + ( ( jf_pattern_endx - jf_chest_underarmx ) * (.5) ), ( jf_button_topy +  5*cm_to_pt ), 'jf_g1' )
           jf_g2x, jf_g2y, jf_g2 = self.GetDot( my_layer, jf_g1x, jf_g1y + 40*cm_to_pt , 'jf_g2' )

           # Jacket Front Pattern path
           Jacket_Front_Pattern = JF_Side +' '+ JF_Armscye_1 +' '+ JF_Armscye_2 +' '+ JF_Shoulder +' '+ JF_Shoulder_to_Hem +' '+ JF_Hem_Allowance +' z' 

           ##################################
           ###  Draw Front Jacket Pattern ###
           ##################################
           my_layer = self.GetNewLayer( pattern_layer, 'Jacket Front')
           Jacket_Front_Layer = my_layer
           self.Path(my_layer,JF_Side_Dart,'dart','Jacket Front Side Dart','')
           self.Path(my_layer,JF_Side_Dart_Foldline,'fold','Jacket Front Side Dart Foldline','')
           self.Path(my_layer,Lapel_Dart,'dart','Collar Dart','')
           self.Path( my_layer, JF_Hem_Line, 'fold', 'Jacket Front Hemline', '' )
           self.Buttons(my_layer,Button_x,Button_y,Button_number,Button_distance,Button_size)
           self.Grainline( my_layer, jf_g1x, jf_g1y, jf_g2x, jf_g2y, 'Jacket Front Grainline', '' )
           self.Path( my_layer, Jacket_Front_Pattern, 'seam', 'Jacket Front Seamline', '' )
           self.Path( my_layer, Jacket_Front_Pattern, 'pattern', 'Jacket Front Cuttingline', '' )
           self.Text( my_layer, jf_shoulder_highx, jf_armhole_curve_4y, 40, 'Company', 'New Day Patterns', '' )
           self.Text( my_layer, jf_shoulder_highx, jf_armhole_curve_4y + 45, 40, 'Pattern number', pattern_number, '' )
           self.Text( my_layer, jf_shoulder_highx, jf_armhole_curve_4y + 2*45, 40, 'Pattern name', 'Jacket Front - B', '' )
           self.Text( my_layer, jf_shoulder_highx, jf_armhole_curve_4y + 3*45, 40, 'Cut', 'Cut 2', '' )
           next_pattern_startx, next_pattern_starty, next_pattern_start = self.GetDot( reference_layer, begin_x, jf_hem_curve_reference_endy + pattern_offset , 'pattern start 3')
           widest_x = jf_pattern_endx

           ####################
           ### Upper Pocket ###
           ####################
           up_bottom_leftx, up_bottom_lefty, up_bottom_left = self.GetDot( my_layer, jf_chest_underarmx + up_width_offset, chest_starty, 'up_bottom_left' )
           up_top_leftx, up_top_lefty, up_top_left = self.GetDot( my_layer, up_bottom_leftx, up_bottom_lefty - up_height, 'up_top_left' )
           up_top_rightx, up_top_righty, up_top_right = self.GetDot( my_layer, up_top_leftx + up_width, up_top_lefty + up_height_offset, 'up_top_right')
           up_bottom_rightx, up_bottom_righty, up_bottom_right = self.GetDot( my_layer, up_top_rightx, up_top_righty + up_height, 'up_bottom_right' )

           x, y = self.XYwithSlope( up_top_leftx, up_top_lefty, up_top_rightx, up_top_righty, seam_allowance, 'normal' )   # add foldline out to seam allowance
           up_fold_1x, up_fold_1y, up_fold_1 = self.GetDot( my_layer, x, y, 'up_fold_1' )
           x, y = self.XYwithSlope( up_top_rightx, up_top_righty, up_top_leftx, up_top_lefty, seam_allowance, 'normal' )   # add foldline out to opposite seam allowance 
           up_fold_2x, up_fold_2y, up_fold_2 = self.GetDot( my_layer, x, y, 'up_fold_2' )

           my_angle = self.AngleFromSlope( up_height_offset, up_width )   # angle defined by height y offset between top 2 point (rise), and pocketx  width (run)
           line_angle = ( math.pi / 2.0 ) - ( 2.0 * my_angle )                      # 90 degrees (pi/2) minus twice the angle
           x, y = self.NewPointFromDistanceAndAngle( up_top_leftx, up_top_lefty, up_height, line_angle )
           up_flap_top_leftx, up_flap_top_lefty, up_flap_top_left = self.GetDot( my_layer, x, y, 'up_flap_top_left' )

           x, y = self.NewPointFromDistanceAndAngle( up_top_rightx, up_top_righty, up_height, line_angle )
           up_flap_top_rightx, up_flap_top_righty, up_flap_top_right = self.GetDot( my_layer, x, y, 'up_flap_top_right' )

           up_g1x, up_g1y, up_g1 = self.GetDot( my_layer, up_flap_top_leftx + 2*cm_to_pt, up_flap_top_lefty + 1*cm_to_pt, 'up_g1' )
           up_g2x, up_g2y, up_g2 = self.GetDot( my_layer, up_g1x, up_bottom_lefty, 'up_g2' )

           UP_Placement = 'M '+ up_bottom_left +' L '+ up_top_left +' '+ up_top_right +' '+ up_bottom_right +' z'
           UP_Pattern   = 'M '+ up_bottom_left +' L '+ up_top_left +' '+ up_flap_top_left +' '+ up_flap_top_right +' '+ up_top_right +' '+ up_bottom_right +' z'
           UP_Foldline  = 'M ' + up_fold_1 +' L '+ up_fold_2
           self.Path( my_layer, UP_Foldline, 'reference', 'Upper Pocket Foldline Reference', '' )
           self.Path( my_layer, UP_Pattern,'reference','Upper Pocket Seamline Reference', '' )
           #################################
           ### Draw Upper Pocket Pattern ###
           ################################# 
           my_layer = self.GetNewLayer( pattern_layer, 'Upper Pocket' )
           dx, dy = -abs( next_pattern_startx - up_top_leftx ), abs( next_pattern_starty - up_flap_top_lefty )
           #up_trans = 'translate( ' + str(dx) +' '+ str(dy) + ' )'
           up_trans = 'translate(' + str(dx) +', '+ str(dy) + ' ) rotate( -15, '+ str( up_top_leftx ) + ', '+ str( up_top_lefty ) + ' )'
           self.Path( Jacket_Front_Layer, UP_Placement, 'fold', 'Upper Pocket placement', '' )
           self.Path( my_layer, UP_Foldline, 'fold', 'Upper Pocket Foldline', up_trans )
           self.Path( my_layer, UP_Pattern,'seam','Upper Pocket Seamline', up_trans )
           self.Path( my_layer, UP_Pattern,'pattern','Upper Pocket Cuttingline', up_trans )
           self.Grainline(my_layer,up_g1x,up_g1y,up_g2x,up_g2y,'Upper Pocket Grainline', up_trans )
           x = up_flap_top_rightx - ( ( up_flap_top_rightx - up_top_leftx ) * .5 )
           y = up_bottom_righty - ( ( up_bottom_righty - up_flap_top_lefty ) * .5 )
           self.Text( my_layer, x, y, 12, 'Company', 'New Day Patterns', up_trans )
           self.Text( my_layer, x, ( y + 15 ), 12, 'Pattern number', pattern_number, up_trans )
           self.Text( my_layer, x, ( y + 2*15 ), 12, 'Pattern name', 'Upper Pocket - C', up_trans )
           self.Text( my_layer, x, ( y + 3*15 ), 12, 'Cut', 'Cut 1', up_trans )
           next_pattern_startx, next_pattern_starty, next_pattern_start = self.GetDot( reference_layer, up_top_rightx + dx + pattern_offset, next_pattern_starty, 'pattern start 4' )

           ####################
           ### Lower Pocket ###
           ####################
           lp_midpointx, lp_midpointy, lp_midpoint = self.GetDot( my_layer, jf_chest_underarmx, jf_chest_underarmy + lp_placement_offset, 'lp_midpoint' )
           m = self.Slope( jf_hem_startx, jf_hem_starty, jf_hem_curve_reference_endx, jf_hem_curve_reference_endy, 'normal' ) # lower pocket is parallel to slanted hem edge
           b = lp_midpointy - ( m * lp_midpointx )
           lp_top_leftx, lp_top_lefty, lp_top_left = self.GetDot( my_layer, lp_midpointx - ( lp_width * (.5) ),  m *  ( lp_midpointx -  ( lp_width  * (.5) ) )  + b, 'lp_top_left' )
           lp_top_rightx, lp_top_righty, lp_top_right = self.GetDot( my_layer, lp_midpointx + ( lp_width * (.5) ),  m *  ( lp_midpointx +  ( lp_width  * (.5) ) )  + b, 'lp_top_left' )          
           lp_bottom_rightx, lp_bottom_righty, lp_bottom_right = self.GetDot( my_layer, lp_top_rightx - lp_slant_offset, lp_top_righty + lp_height , 'lp_bottom_right' )
           lp_bottom_leftx, lp_bottom_lefty, lp_bottom_left = self.GetDot( my_layer, lp_top_leftx - lp_slant_offset, lp_top_lefty + lp_height , 'lp_bottom_left' )
           b = lp_bottom_righty - ( m * lp_bottom_rightx )
           lp_curve_endx, lp_curve_endy, lp_curve_end = self.GetDot( my_layer, lp_bottom_rightx - ( lp_width  * (.25) ), b + ( m * ( lp_bottom_rightx - ( lp_width * (.25) ) ) ), 'lp_curve_end' )
           lp_flap_top_leftx,  lp_flap_top_lefty,  lp_flap_top_left  = self.GetDot( my_layer, lp_top_leftx,  lp_top_lefty - lp_flap_height, 'lp_flap_top_left' )
           lp_flap_top_rightx, lp_flap_top_righty, lp_flap_top_right = self.GetDot( my_layer, lp_top_rightx, lp_top_righty - lp_flap_height, 'lp_flap_top_right' )
           lp_foldpoint1x, lp_foldpoint1y, lp_foldpoint1 = self.GetDot( my_layer, lp_top_leftx - seam_allowance, lp_top_lefty, 'lp_foldpoint1' ) #lp_foldpoint1 extends past seam allowance 
           lp_foldpoint2x, lp_foldpoint2y, lp_foldpoint2 = self.GetDot( my_layer, lp_top_rightx + seam_allowance, lp_top_righty, 'lp_foldpoint2' ) #lp_foldpoint2 extends past seam allowance 
           lp_G1x, lp_G1y, lp_G1 = self.GetDot( my_layer, lp_midpointx, ( lp_midpointy + 1*cm_to_pt ), 'lp_G1' )
           lp_G2x, lp_G2y, lp_G2 = self.GetDot( my_layer, ( lp_bottom_leftx + ( abs( lp_bottom_leftx - lp_bottom_rightx ) * 0.5 ) ), ( lp_bottom_lefty - 1*cm_to_pt ), 'lp_G2' )

           LPocket_Placement = 'M '+ lp_top_left +' L '+ lp_top_right +' Q '+ lp_bottom_right +' '+ lp_curve_end+ ' L '+ lp_bottom_left+ ' z'
           LPocket_Pattern   = 'M '+ lp_top_left +' L '+ lp_flap_top_left +' '+ lp_flap_top_right +' '+ lp_top_right +' Q '+ lp_bottom_right +' '+ lp_curve_end +' L '+ lp_bottom_left + ' z' 
           LPocket_Foldline  = 'M '+ lp_foldpoint1 +' L '+ lp_foldpoint2
           self.Path( my_layer, LPocket_Foldline, 'reference', 'Lower Pocket Foldline Reference', '' )
           self.Path( my_layer, LPocket_Pattern,'reference','Lower Pocket Seamline Reference', '' )
           #################################
           ### Draw Lower Pocket Pattern ###
           #################################  
           my_layer = self.GetNewLayer(pattern_layer,'Lower Pocket')
           dx, dy = -abs( next_pattern_startx - lp_flap_top_leftx ), abs( next_pattern_starty - lp_flap_top_righty )
           lp_trans = 'translate( ' + str(dx) +' '+ str(dy) + ' )' 
           self.Path( Jacket_Front_Layer ,LPocket_Placement,'dart','Lower Pocket placement','')         
           self.Path(my_layer,LPocket_Foldline,'fold','Lower Pocket Foldline', lp_trans )
           self.Path(my_layer,LPocket_Pattern,'seam','Lower Pocket Seamline', lp_trans )
           self.Path(my_layer,LPocket_Pattern,'pattern','Lower Pocket Cuttingline', lp_trans)
           self.Grainline(my_layer,lp_G1x,lp_G1y,lp_G2x,lp_G2y,'Lower Pocket Grainline', lp_trans )
           x =  lp_flap_top_rightx - ( ( lp_flap_top_rightx - lp_flap_top_leftx ) * .8 )
           y =  lp_bottom_righty - ( ( lp_bottom_righty - lp_flap_top_righty ) * .7 )
           self.Text( my_layer, x, y, 20, 'Company', 'New Day Patterns', lp_trans )
           self.Text( my_layer, x, ( y + 25 ), 20, 'Pattern number', pattern_number, lp_trans )
           self.Text( my_layer, x, ( y + 2*25 ), 20, 'Pattern name', 'Lower Pocket - D', lp_trans )
           self.Text( my_layer, x, ( y + 3*25 ), 20, 'Cut', 'Cut 2', lp_trans )
           next_pattern_startx, next_pattern_starty, next_pattern_start = self.GetDot( reference_layer, lp_flap_top_rightx + dx + pattern_offset, next_pattern_starty, 'pattern start 5' )

           ##############
           ### Collar ###
           ##############
           my_layer = reference_layer
           rise = -3*cm_to_pt
           run  = 1*cm_to_pt
           collar_back_length  = self.LineLength( napex, napey, back_shoulder_highx, back_shoulder_highy )
           collar_jf_length = abs( jf_lapel_pointx - jf_lapel_neck_intersectx ) - 3*cm_to_pt - ( jf_lapel_dart_width / 2.0 )
           collar_neck_curvex, collar_neck_curvey, collar_neck_curve = jf_lapel_neck_intersectx, jf_lapel_neck_intersecty, str( jf_lapel_neck_intersectx ) + ' ' + str( jf_lapel_neck_intersecty )
           x, y = self.XY( collar_neck_curvex, collar_neck_curvey, jf_lapel_pointx, jf_lapel_pointy, -collar_jf_length )
           collar_frontx,  collar_fronty,  collar_front  = self.GetDot( my_layer, x, y, 'collar_front' )
           collar_pointx, collar_pointy, collar_point = self.GetDot( my_layer, ( collar_frontx + run ), ( collar_fronty + rise ), 'collar_point' )
           collar_neck_pointx, collar_neck_pointy, collar_neck_point = jf_shoulder_highx,jf_shoulder_highy, str(jf_shoulder_highx) + ' ' + str(jf_shoulder_highy)
           x, y = self.XY(collar_neck_pointx, collar_neck_pointy, collar_neck_curvex, collar_neck_curvey, collar_back_length )
           collar_endx, collar_endy, collar_end = self.GetDot( my_layer, x, y, "collar_end" )
           x, y = self.XYwithSlope ( collar_endx, collar_endy, collar_neck_curvex, collar_neck_curvey, 3*cm_to_pt, 'perpendicular' )
           collar_bottomx, collar_bottomy, collar_bottom = self.GetDot( my_layer, x, y, "collar_bottom" )
           a, b = self.XYwithSlope( collar_endx, collar_endy, collar_neck_curvex, collar_neck_curvey, -3*cm_to_pt, 'perpendicular' )
           x, y = self.XYwithSlope( a, b, collar_endx, collar_endy, -0.6*cm_to_pt, 'perpendicular' )
           collar_topx, collar_topy, collar_top = self.GetDot( my_layer, x, y, "collar_top" )
           control_length = self.LineLength( collar_neck_curvex, collar_neck_curvey, collar_endx, collar_endy ) * (0.25) 
           c1x, c1y, c1 = self.GetDot( my_layer, collar_neck_curvex - ( abs( collar_neck_curvex - collar_bottomx )/3  ), collar_neck_curvey, 'c1' )
           a, b = self.XYwithSlope( collar_bottomx, collar_bottomy, collar_endx, collar_endy, 100, 'perpendicular' )
           x, y = self.XY( collar_bottomx, collar_bottomy, a, b, ( abs( collar_neck_curvey - collar_bottomy ) / 3 ) )
           c2x, c2y, c2  = self.GetDot( my_layer, x, y, 'c2' )
           x, y = self.XY( collar_neck_curvex, collar_neck_curvey, jf_chest_endx, jf_chest_endy, control_length )  
           c3x, c3y, c3 = self.GetDot( my_layer, x, y, 'c3' )  
           x, y = self.XYwithSlope( collar_endx, collar_endy, collar_topx, collar_topy, -control_length, 'perpendicular' )   
           c4x, c4y, c4 = self.GetDot( my_layer, x, y, 'c4' )
           cg1x, cg1y, cg1 = collar_topx + ( abs( collar_neck_curvex - collar_topx ) / 2 ), collar_bottomy + 1*cm_to_pt, str( collar_bottomx + ( abs( collar_neck_curvex - collar_topx ) * 0.5 ) ) + ' '+ str( collar_bottomy )
           cg2x, cg2y, cg2 = cg1x, cg1y + ( 6*cm_to_pt ) , str( cg1x ) + ' ' + str( cg1y + ( 6*cm_to_pt ) )
           Collar = 'M ' + collar_top +' '+ ' L '+ collar_point + ' '+ collar_front +' '+ collar_neck_curve +' C '+ c1 +' '+ c2 +' '+ collar_bottom +' L '+ collar_end + ' z'
           Collar_Roll  = 'M ' + collar_neck_curve + ' C ' + c3 + ' ' + c4 + ' ' + collar_end
           self.Path( my_layer, Collar, 'reference', 'Collar', '' )
           self.Path( my_layer, Collar_Roll, 'reference', 'Collar Roll Line', '' )
           ###########################
           ### Draw Collar Pattern ###
           ########################### 
           my_layer = self.GetNewLayer( pattern_layer, 'Collar' )
           dx, dy =  -abs( next_pattern_startx - collar_topx ), abs( next_pattern_starty - collar_topy ) 
           c_trans = 'translate(' + str(dx) +', '+ str(dy) + ' ) rotate( -35, '+ str( collar_topx ) + ', '+ str( collar_topy ) + ' )'
           self.Path( my_layer, Collar, 'seam', 'Collar Seamline', c_trans )  
           self.Path( my_layer, Collar, 'pattern', 'Collar Cutting Line', c_trans )
           self.Path( my_layer, Collar_Roll, 'fold', 'Collar Roll Line', c_trans)
           self.Grainline( my_layer, cg1x, cg1y, cg2x, cg2y, 'Grainline', c_trans )
           x =  collar_neck_curvex - ( ( collar_neck_curvex - collar_endx ) * .15 )
           y =  collar_neck_curvey - ( ( collar_neck_curvey - collar_topy ) * .4 )
           self.Text( my_layer, x, y, 20, 'Company', 'New Day Patterns', c_trans )
           self.Text( my_layer, x, ( y + 25 ), 20, 'Pattern number', pattern_number, c_trans )
           self.Text( my_layer, x, ( y + 2*25 ), 20, 'Pattern name', 'Collar - E', c_trans )
           self.Text( my_layer, x, ( y + 3*25 ), 20, 'Cut', 'Cut 2', c_trans )
           next_pattern_startx, next_pattern_starty, next_pattern_start = self.GetDot( reference_layer, begin_x, next_pattern_starty + abs( collar_topy - collar_bottomy ) + pattern_offset, 'pattern start 6' )
           lowest_y = collar_topy
           ####################
           ### Upper Sleeve ###
           ####################
           my_layer = reference_layer
           uppersleeve_begin_x = next_pattern_startx    
           uppersleeve_begin_y = next_pattern_starty

           SA1x, SA1y, SA1 = self.GetDot( my_layer, uppersleeve_begin_x, uppersleeve_begin_y, 'SA1' )
           SB1x,SB1y,SB1=self.GetDot(my_layer, SA1x,(SA1y+((chest/16)-2*cm_to_pt)),'SB1')
           SC1x,SC1y,SC1=self.GetDot(my_layer,SA1x,SB1y+(chest_starty-sleeve_back_balance_pointy),'SC1')
           SD1x,SD1y,SD1=self.GetDot(my_layer,SA1x,SC1y+19*cm_to_pt,'SD1')
           SF1x,SF1y,SF1=self.GetDot(my_layer,SA1x,SB1y+(sleeve_length),'SF1')

           c1x,c1y,c1=self.GetDot(my_layer,SC1x,SC1y-(abs(SC1y-SB1y)*(.3)),'c1')
           x1,y1=SA1x-100,SA1y-100
           x2,y2=self.XYwithSlope(SA1x, SA1y,x1,y1,3*cm_to_pt,'normal')
           SA2x,SA2y,SA2=self.GetDot(my_layer,x2,y2,'SA2')
           SA3x,SA3y,SA3=self.GetDot(my_layer,SA1x+(((chest/4)-(3*cm_to_pt))/2),SA1y,'SA3')
           SA4x,SA4y,SA4=self.GetDot(my_layer,(SA1x+(chest/4-3*cm_to_pt)),SA1y,'SA4')

           SB2x,SB2y,SB2=self.GetDot(my_layer,(SA4x-(4*cm_to_pt)),SB1y,'SB2')
           SB3x,SB3y,SB3=self.GetDot(my_layer,SA4x,SB1y,'SB3')
           SB4x,SB4y,SB4=self.GetDot(my_layer,(SB3x+8*cm_to_pt),SB1y,'SB4')
           SB5x,SB5y,SB5=self.GetDot(my_layer,(SB4x+1.3*cm_to_pt),SB1y,'SB5')
           SB6x,SB6y,SB6=self.GetDot(my_layer,SB4x+(SA3x-SA1x),SB4y+((chest_starty-sleeve_back_balance_pointy)-(2*cm_to_pt)),'SB6')
           SB7x,SB7y,SB7=self.GetDot(my_layer,SB6x+((SB6x-SB4x)/2),SB6y+1*cm_to_pt,'SB7')
           SB8x,SB8y,SB8=self.GetDot(my_layer,SB6x+((SB6x-SB4x)),SB4y+((chest_starty-sleeve_back_balance_pointy)-(jf_chest_underarmy-jf_armhole_curve_3y)),'SB8')

           c1x,c1y,c1=self.GetDot(my_layer,SB1x+abs(SA2x-SB1x)*(.6),SB1y-abs(SA2y-SB1y)*(.7),'c1')
           c2x,c2y,c2=self.GetDot(my_layer,SA2x+abs(SA3x-SA2x)*(.25),SA2y-abs(SA3y-SA2y)*(.6),'c2')
           c3x,c3y,c3=self.GetDot(my_layer,SA2x+abs(SA3x-SA2x)*(.6),SA3y-((.5)*cm_to_pt),'c3')
           c4x,c4y,c4=self.GetDot(my_layer,SA3x+abs(SA3x-SB2x)*(.25),SA3y,'c4')
           c5x,c5y,c5=self.GetDot(my_layer,SA3x+abs(SA3x-SB2x)*(.7),SA3y+abs(SA3y-SB2y)*(.45),'c5')
           SC2x,SC2y,SC2=self.GetDot(my_layer,SC1x+((SA4x-SA1x)/2),SC1y,'SC2')
           SC3x,SC3y,SC3=self.GetDot(my_layer,SA4x,SC1y,'SC3')
           SC4x,SC4y,SC4=self.GetDot(my_layer,SA4x,SC3y-(jf_chest_underarmy-jf_armhole_curve_3y),'SC4')
           SC5x,SC5y,SC5=self.GetDot(my_layer,SB4x,SC1y,'SC5')
           SC6x,SC6y,SC6=self.GetDot(my_layer,SC5x+1*cm_to_pt,SC1y,'SC6')
           SC7x,SC7y,SC7=self.GetDot(my_layer,SB6x,SC1y,'SC7')
           SC8x,SC8y,SC8=self.GetDot(my_layer,SB8x,SC1y,'SC8')

           c6x,c6y,c6=self.GetDot(my_layer,SB2x+abs(SC4x-SB2x)*(.25),SB2y+abs(SC4x-SB2x)*(.25),'c6')
           c7x,c7y,c7=self.GetDot(my_layer,SB2x+abs(SC4x-SB2x)*(.85),SB2y+abs(SC4y-SB2y)*(.5),'c7')
           Upper_Sleeve_Curve=' Q '+c1+' '+SA2+' C '+c2+' '+c3+','+SA3+' C '+c4+' '+c5+','+SB2+' C '+c6+','+c7+' '+SC4

           SD2x,SD2y,SD2=self.GetDot(my_layer,SD1x+1*cm_to_pt,SD1y,'SD2')
           SD3x,SD3y,SD3=self.GetDot(my_layer,SA4x-1.3*cm_to_pt,SD1y,'SD3')
           SD4x,SD4y,SD4=self.GetDot(my_layer,SA4x,SD1y,'SD4')
           SD5x,SD5y,SD5=self.GetDot(my_layer,SB4x,SD1y,'SD5')
           SD6x,SD6y,SD6=self.GetDot(my_layer,SD5x+1*cm_to_pt,SD1y,'SD6')
           SD7x,SD7y,SD7=self.GetDot(my_layer,SB8x-1.3*cm_to_pt,SD1y,'SD7')
           SD8x,SD8y,SD8=self.GetDot(my_layer,SB8x,SD1y,'SD8')

           SF1x,SF1y,SF1=self.GetDot(my_layer,SA1x,SB1y+sleeve_length,'SF1')
           SF2x,SF2y,SF2=self.GetDot(my_layer,SF1x+7.5*cm_to_pt,SF1y,'SF2')
           SF3x,SF3y,SF3=self.GetDot(my_layer,SA4x,SF1y,'SF3')
           SF4x,SF4y,SF4=self.GetDot(my_layer,SF3x,SF3y-2.5*cm_to_pt,'SF4')
           x1,y1=self.XYwithSlope(SF4x,SF4y,SF2x,SF2y,2*cm_to_pt,'normal')
           SF5x,SF5y,SF5=self.GetDot(my_layer,x1,y1,'SF5')
           SF6x,SF6y,SF6=self.GetDot(my_layer,SB4x,SF1y,'SF6')
           SF7x,SF7y,SF7=self.GetDot(my_layer,SF6x+7.5*cm_to_pt,SF1y,'SF7')
           SF8x,SF8y,SF8=self.GetDot(my_layer,SB8x,SF1y,'SF8')
           SF9x,SF9y,SF9=self.GetDot(my_layer,SF8x,SF8y-2.5*cm_to_pt,'SF9')
           x1,y1=self.XYwithSlope(SF9x,SF9y,SF7x,SF7y,2*cm_to_pt,'normal')
           SF10x,SF10y,SF10=self.GetDot(my_layer,x1,y1,'SF10')

           # Reference Lines
           my_path='M '+SA1+' L '+SF1
           self.Path(my_layer,my_path,'reference','Sleeve Length Reference SA1SF1','')
           my_path='M '+SA1+' L '+SA4
           self.Path(my_layer,my_path,'reference','Sleeve Top Reference SA1SA4','')
           my_path='M '+SA1+' L '+SA2
           self.Path(my_layer,my_path,'reference','Sleeve Corner Reference SA1SA2','')
           my_path='M '+SB1+' L '+SB5+' '+SB6+' '+SB7+' '+SB8
           self.Path(my_layer,my_path,'reference','Sleeve Reference SB','')
           my_path='M '+SA3+' L '+SC2
           self.Path(my_layer,my_path,'reference','Sleeve Cap Reference SA3SC2 ','')
           my_path='M '+SC1+' L '+SC8
           self.Path(my_layer,my_path,'reference','Sleeve Reference SC','')
           my_path='M '+SD1+' L '+SD8
           self.Path(my_layer,my_path,'reference','Sleeve Reference Line SD','') 
           my_path='M '+SF1+' L '+SF8
           self.Path(my_layer,my_path,'reference','Sleeve Reference SF','')
           my_path='M '+SA4+' L '+SF3
           self.Path(my_layer,my_path,'reference','Sleeve Side Reference SA4SF3','')
           my_path='M '+SB4+' L '+SF6
           self.Path(my_layer,my_path,'reference','Sleeve Side Reference SB4SF6','')
           my_path='M '+SB6+' L '+SC7
           self.Path(my_layer,my_path,'reference','Sleeve Cap Reference SB6SC7','')
           my_path='M '+SB8+' L '+SF8
           self.Path(my_layer,my_path,'reference','Sleeve Side Reference SB8SF8','')

           # Cuff Placement
           SE1x, SE1y, SE1 = self.GetDot( my_layer, SF2x - (2.5*cm_to_pt), SF2y - (10*cm_to_pt), 'SE1' )
           SE2x, SE2y, SE2 = self.GetDot( my_layer, SF3x + (0.5*cm_to_pt), SF3y - (12.5*cm_to_pt), 'SE2' )
           SE3x, SE3y, SE3 = self.GetDot( my_layer, SF7x - (2.5*cm_to_pt), SF7y - (10*cm_to_pt),'SE3' )
           SE4x, SE4y, SE4 = self.GetDot( my_layer, SF8x + (0.5*cm_to_pt), SF8y - (12.5*cm_to_pt),'SE4' )
           Uppersleeve_Cuff_Placement_Line = 'M ' + SE1 + ' L ' + SE2
           Undersleeve_Cuff_Placement_Line = 'M ' + SE3 + ' L '+ SE4
           Uppersleeve_Cuff_Fold_Line      = 'M ' + SF2 + ' L ' + SF5

           # Hem Line & Allowance --> Extend the cuff, reflected about the fold line
           central_angle1    = self.AngleFromSlope( abs( SE2y - SE1y ), abs( SE2x - SE1x ) )
           cuff_height       = self.LineLength( SE2x, SE2y, SF5x, SF5y )
           central_angle2    = self.AngleFromSlope( abs( SF5y - SF2y ), abs( SF5x - SF2x ) )
           line_angle        = self.AngleFromSlope( abs( SE2y - SF5y ), abs( SE2x - SF5x ) )
           mirror_line_angle = central_angle2 - line_angle
           x, y = self.NewPointFromDistanceAndAngle( SF5x, SF5y, cuff_height, mirror_line_angle )
           hem2x, hem2y, hem2 = self.GetDot( my_layer, x, y, 'hem2' )
           angle3 = central_angle1 + central_angle2
           x, y = self.NewPointFromDistanceAndAngle( hem2x, hem2y, -self.LineLength( SE2x, SE2y, SE1x, SE1y ), angle3 )
           hem1x, hem1y, hem1 = self.GetDot( my_layer, x, y, 'hem1' )
           x, y   = self.XYwithSlope( hem1x, hem1y, hem2x, hem2y, 1*cm_to_pt, 'normal' )
           uppersleeve_hem1x, uppersleeve_hem1y, uppersleeve_hem1 = self.GetDot( my_layer, x, y, 'uppersleeve_hem1' )
           x, y   = self.XYwithSlope( hem2x, hem2y, hem1x, hem1y, 1*cm_to_pt, 'normal' )
           uppersleeve_hem2x, uppersleeve_hem2y, uppersleeve_hem2 = self.GetDot( my_layer, x, y, 'uppersleeve_hem2' )
           x, y = self.XYwithSlope( uppersleeve_hem1x, uppersleeve_hem1y, uppersleeve_hem2x, uppersleeve_hem2y, -( self.LineLength( uppersleeve_hem1x, uppersleeve_hem1y, uppersleeve_hem2x, uppersleeve_hem2y ) * (.7) ), 'normal' )
           hem1ax, hem1ay, hem1a = self.GetDot( my_layer, x, y, 'hem1a' )
           x, y = self.XYwithSlope( uppersleeve_hem1x, uppersleeve_hem1y, SF2x, SF2y, -( cuff_height * (.8) ), 'normal' )
           hem1bx, hem1by, hem1b = self.GetDot( my_layer, x, y, 'hem1b' )
           Uppersleeve_Hem_Line = ' L ' + uppersleeve_hem2 + ' ' + hem1a + ' Q ' + uppersleeve_hem1 + ' ' + hem1b

           # Sleeve Side 1 SF2-SB1
           x1,y1=self.XYwithSlope(SE1x,SE1y,SF2x,SF2y,abs(SD2y-SE1y)*(.25),'normal')
           c1x,c1y,c1=self.GetDot(my_layer,x1,y1,'c1')
           c2x,c2y,c2=self.GetDot(my_layer,SD2x+15,SE1y-abs(SE1y-SD2y)*(.8),'c2')
           c3x,c3y,c3=self.GetDot(my_layer,SD2x-abs(SD2x-SC1x)*(.4),SD2y-abs(SD2y-SC1y)*(.18),'c3')
           c4x,c4y,c4=self.GetDot(my_layer,SC1x,SD2y-abs(SD2y-SC1y)*(.9),'c4')
           Upper_Sleeve_Side_1='M '+SF2+' L '+SE1+ ' C '+c1+' '+c2+' '+SD2+' C '+c3+' '+c4+' '+SC1+' L '+SB1

           # Sleeve Side 2 SC4-SD3
           c1x,c1y,c1=self.GetDot(my_layer,SC4x-abs(SC4x-SD3x)*(.5),SC4y+abs(SC4y-SD3y)*(.15),'c1')
           c2x,c2y,c2=self.GetDot(my_layer,SD3x,SC3y+abs(SC4y-SD3y)*(.8),'c2')
           c3x,c3y,c3=self.GetDot(my_layer,SD3x,SD3y+abs(SD3y-SE2y)*(.3),'c3')
           c4x,c4y,c4=self.GetDot(my_layer,SD3x+abs(SD3x-SE2x)*(.5),SD3y+abs(SD3y-SE2y)*(.8),'c4')
           Upper_Sleeve_Side_2=' C '+c1+' '+c2+' '+SD3+' C '+c3+' '+c4+' '+SE2+' L '+SF5

           # Grainline
           G1x,G1y,G1=self.GetDot(my_layer,SC2x,SC2y+6*cm_to_pt,'G1')
           G2x,G2y,G2=self.GetDot(my_layer,SC2x,SC1y+40*cm_to_pt,'G2')

           #################################
           ### Draw Upper Sleeve Pattern ###
           #################################
           my_layer = self.GetNewLayer( pattern_layer, 'Upper Sleeve' ) 
           Upper_Sleeve_Pattern = Upper_Sleeve_Side_1 +' '+ Upper_Sleeve_Curve +' '+ Upper_Sleeve_Side_2 +' '+ Uppersleeve_Hem_Line + ' z'
           self.Path( my_layer, Upper_Sleeve_Pattern, 'seam', 'Upper Sleeve Seamline', '' )
           self.Path( my_layer, Upper_Sleeve_Pattern, 'pattern', 'Upper Sleeve Cuttingline', '' )
           self.Path( my_layer, Uppersleeve_Cuff_Placement_Line, 'fold', 'Upper Sleeve Cuff Placement Line', '' )
           self.Path( my_layer, Uppersleeve_Cuff_Fold_Line, 'fold', 'Upper Sleeve Cuff Fold Line', '' )
           self.Grainline( my_layer, G1x, G1y, G2x, G2y, 'Upper Sleeve Grainline', '' )
           x = SA3x
           y =  SC4y 
           self.Text( my_layer, x, y, 40, 'Company', 'New Day Patterns', '' )
           self.Text( my_layer, x, ( y + 45 ), 40, 'Pattern number', pattern_number, '' )
           self.Text( my_layer, x, ( y + 2*45 ), 40, 'Pattern name', 'Upper Sleeve - F', '' )
           self.Text( my_layer, x, ( y + 3*45 ), 40, 'Cut', 'Cut 2', '' )

           ####################
           ### Under Sleeve ###
           ####################
           my_layer=reference_layer
           # Sleeve Side 3 SF7-SE3-SD6-SC6-SB5
           x1,y1=self.XYwithSlope(SE3x,SE3y,SF7x,SF7y,abs(SD6y-SE3y)*(.25),'normal')
           c1x,c1y,c1=self.GetDot(my_layer,x1,y1,'c1')
           c2x,c2y,c2=self.GetDot(my_layer,SD6x+15,SE3y-abs(SE3y-SD6y)*(.8),'c2')
           c3x,c3y,c3=self.GetDot(my_layer,SD6x-10,SD6y-abs(SD6y-SC6y)*(.4),'c3')
           c4x,c4y,c4=self.GetDot(my_layer,SC6x-5,SD6y-abs(SD6y-SC6y)*(.85),'c4')
           Sleeve_Side_3='M '+SF7+' L '+SE3+ ' C '+c1+' '+c2+' '+SD6+' C '+c3+' '+c4+' '+SC6+' L '+SB5
           #Sleeve Underarm SB5-SB6-SB7-SB8
           c1x,c1y,c1=self.GetDot(my_layer,SB5x+abs(SB5x-SB6x)*(.6),SB5y+abs(SB5y-SB6y)*(.8),'c1')
           c2x,c2y,c2=self.GetDot(my_layer,SB6x+abs(SB6x-SB7x)*(.5),SB7y+10,'c2')
           c3x,c3y,c3=self.GetDot(my_layer,SB7x+abs(SB7x-SB8x)*(.8),SB7y-abs(SB7y-SB8y)*(.4),'c3')
           Underarm=' Q '+c1+' '+SB6+' Q '+c2+' '+SB7+' Q '+' '+c3+' '+SB8
           #Sleeve Side 4 SB8-SC8-SD7-SE4-SF10
           c1x,c1y,c1=self.GetDot(my_layer,SC8x-abs(SC8x-SD7x)*(.5),SC8y+abs(SC8y-SD7y)*(.15),'c1')
           c2x,c2y,c2=self.GetDot(my_layer,SD7x,SC8y+abs(SC8y-SD7y)*(.8),'c2')
           c3x,c3y,c3=self.GetDot(my_layer,SD7x,SD7y+abs(SD7y-SE4y)*(.3),'c3')
           c4x,c4y,c4=self.GetDot(my_layer,SD7x+abs(SD7x-SE4x)*(.5),SD7y+abs(SD7y-SE4y)*(.8),'c4')
           #Sleeve_Side_4=' L '+SC8+' '+' C '+c1+' '+c2+' '+SD7+' C '+' '+c3+' '+c4+' '+SE4+' L '+SF10
           Sleeve_Side_4=' C '+c1+' '+c2+' '+SD7+' C '+' '+c3+' '+c4+' '+SE4+' L '+SF10

           # Hem Line & Allowance --> Extend the cuff
           central_angle1 = self.AngleFromSlope( abs(SE4y-SE3y ), abs(SE4x-SE3x))
           cuff_height = self.LineLength(SE4x,SE4y,SF10x,SF10y)
           central_angle2 = self.AngleFromSlope(abs(SF10y-SF7y),abs(SF10x-SF7x))
           line_angle = self.AngleFromSlope( abs(SE4y-SF10y),abs(SE4x-SF10x))
           mirror_line_angle = central_angle2 - line_angle
           x, y = self.NewPointFromDistanceAndAngle( SF10x, SF10y, cuff_height, mirror_line_angle)
           hem2x, hem2y, hem2 = self.GetDot( my_layer, x, y, 'hem2' )
           angle3 = central_angle1 + central_angle2
           x, y   = self.NewPointFromDistanceAndAngle( hem2x, hem2y, -self.LineLength( SE4x, SE4y, SE3x, SE3y ), angle3 )
           hem1x, hem1y, hem1 = self.GetDot( my_layer, x, y, 'hem1' )
           x, y   = self.XYwithSlope( hem2x, hem2y, hem1x, hem1y, 1*cm_to_pt, 'normal' )
           undersleeve_hem2x, undersleeve_hem2y, undersleeve_hem2 = self.GetDot( my_layer, x, y, 'undersleeve_hem2' )
           x, y   = self.XYwithSlope( hem1x, hem1y, hem2x, hem2y, 1*cm_to_pt, 'normal' )
           undersleeve_hem1x, undersleeve_hem1y, undersleeve_hem1 = self.GetDot( my_layer, x, y, 'undersleeve_hem1' )
           x, y = self.XYwithSlope( undersleeve_hem1x, undersleeve_hem1y, undersleeve_hem2x, undersleeve_hem2y, -( self.LineLength( undersleeve_hem1x, undersleeve_hem1y, undersleeve_hem2x, undersleeve_hem2y ) * (.7) ), 'normal' )
           hem1ax, hem1ay, hem1a = self.GetDot( my_layer, x, y, 'hem1a' )
           x, y = self.XYwithSlope( undersleeve_hem1x, undersleeve_hem1y, SF7x, SF7y, -( cuff_height * (.8) ), 'normal' )
           hem1bx, hem1by, hem1b = self.GetDot( my_layer, x, y, 'hem1b' )
           Undersleeve_Hem_Line = ' L ' + undersleeve_hem2 + ' ' + hem1a + ' Q ' + undersleeve_hem1 + ' ' + hem1b
           Undersleeve_Cuff_Fold_Line = 'M '+ SF7 +' L '+ SF10

           #Under Sleeve Grainline
           G1x,G1y,G1=self.GetDot(my_layer,SC7x,SC7y+15*cm_to_pt,'G1')
           G2x,G2y,G2=self.GetDot(my_layer,G1x,G1y+40*cm_to_pt,'G2')
           ############################
           ### Under Sleeve Pattern ###
           ############################
           my_layer = self.GetNewLayer( pattern_layer, 'Under Sleeve' )
           Under_Sleeve_Pattern = Sleeve_Side_3 +' '+ Underarm + ' '+ Sleeve_Side_4 +' '+ Undersleeve_Hem_Line + ' z '
           self.Path( my_layer, Under_Sleeve_Pattern, 'seam', 'Seamline', '' )
           self.Path( my_layer, Under_Sleeve_Pattern, 'pattern', 'Cuttingline', '' )
           self.Path( my_layer, Undersleeve_Cuff_Placement_Line, 'fold', 'Cuff Placement Line', '' )
           self.Path( my_layer, Undersleeve_Cuff_Fold_Line, 'fold', 'Cuff Fold Line', '' )
           self.Grainline( my_layer, G1x, G1y, G2x, G2y, 'Grainline', '' )
           x = SC7x
           y = SC7y + 6*cm_to_pt 
           self.Text( my_layer, x, y, 40, 'Company', 'New Day Patterns', '' )
           self.Text( my_layer, x, ( y + 45 ), 40, 'Pattern number', pattern_number, '' )
           self.Text( my_layer, x, ( y + 2*45 ), 40, 'Pattern name', 'Upper Sleeve - F', '' )
           self.Text( my_layer, x, ( y + 3*45 ), 40, 'Cut', 'Cut 2', '' )
           highest_y = undersleeve_hem1y
           ###################################
           ### Resize Document, Reset View ###
           ###################################
           self.layer = reference_layer
           height = border + abs( highest_y - lowest_y ) + border
           width  = border + abs( widest_x - begin_x ) + border
           self.svg_svg( str( width ), str( height ), str( border ) )

my_effect = DrawJacket()
my_effect.affect()
